...
1. **Cookie 规格（FROZEN）**：名 `__Host-lqx_guest`；值 = signedToken（`stok_v1...`，契约 03）；
   `HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=43200`（= **12h**，§6 裁定：`GUEST_COOKIE_MAX_AGE <= 12h`，
   **禁止 24h**）；**无 Domain**（`__Host-` 前缀本身要求 Secure + Path=/ + 无 Domain，浏览器强制校验）；不设 Partitioned。
2. Cookie = **token 传输层**，不承载任何业务数据；服务器对 guest 只把 Cookie 当 token 来源（契约 03-5 冲突矩阵）。
3. 只由铸 token 端点 Set-Cookie；过期由 Max-Age 自然清除；客户端永不读写该 Cookie。
4. **CSRF 策略**：
...
6. **双来源冲突语义（§8 裁定，FROZEN，详见契约 03-5）**：Cookie+Header 同身份 → accept；不同身份 → **FAIL CLOSED**；
   任一 invalid（另一存在）→ **FAIL CLOSED**；错误码 `TOKEN_CONTEXT_CONFLICT`；**无 silent fallback**。
...
- D5（§13 裁定）：无加密黑名单，但 isActive/记录存在 = 服务端强制撤销（契约 03 DESIGN_BOUNDARY）。
  本契约范围内风险由 12h TTL + HttpOnly/Secure/SameSite + IP 兜底（契约 08）覆盖。

## FUTURE_IMPLEMENTATION

- 铸 token 端点 Set-Cookie 组装（契约 03）；`getAccessContext` Cookie 来源 + 冲突矩阵；Origin 校验中间层；
