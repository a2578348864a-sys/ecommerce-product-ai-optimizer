...
### Phase 3C：Confirm command Characterization（测试/文档候选）

Phase 3C 只新增 `OpportunitiesForm.source-confirm-command.test.ts` 并更新治理文档，不修改 `OpportunitiesForm.tsx`、request adapter、API、Candidate/Task写入、权限、Storage或任何生产代码。46项挂载测试冻结：
...
证据显示常规 saving DOM 能阻止后续点击，但同一事件周期内两个公开 DOM click 可发出两个权威 POST；任一 finally 会关闭共享 saving，旧 refresh 也可覆盖较新提示。Confirm 没有 generation、single-flight、幂等 key 或 AbortController，服务端幂等性为 `UNKNOWN`。这些是已记录风险，不是 Phase 3C 修复结果。
...
|18|Confirm Characterization（Phase 3C 候选）|公开交互 + 虚假身份 + 内存 fetch → 请求/刷新/竞态证据|父组件 command|测试内存 I/O；生产不变|重复权威 POST、共享 saving、服务端幂等性 UNKNOWN|
...
先保护 command 行为，再逐 endpoint 建立窄 adapter。Phase 3A adapter 与 Phase 3B preview generation 已进入 production main；state、confirm、签名、crawler、API 和选择规则均未移动。Phase 3C 候选只建立 Confirm 请求、权限 UI、refresh、错误和重复提交 Characterization，不修改生产实现。Phase 3D 尚未实施，必须基于 Phase 3C 的 P1/UNKNOWN 边界另行决策。
