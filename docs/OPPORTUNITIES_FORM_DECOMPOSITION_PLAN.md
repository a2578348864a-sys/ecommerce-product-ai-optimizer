...
### Phase 2A：Candidate pool 派生计数 selector（候选已完成）
...
|7|Candidate pool counts（Phase 2A 候选已完成）|只读 Candidate pool → 六字段计数|容器 memo|无|未知状态/converted Task 语义漂移|
|8|Decision badges|Candidate presentation → JSX|容器|无|Evidence/R2.2 标签漂移|
|9|Decision View Model|pool + Task links + mode → rows|纯 module|无|authority 条件遗漏|
|10|Source import view|view model + commands → JSX|容器|无直接 fetch|preview/confirm 混淆|
|11|Candidate list/detail|rows + selection + commands → JSX|容器|portal 另行处理|DOM/菜单行为|
|12|Storage recovery module|cache adapter → hydration result|专用 Hook/module|localStorage|覆盖顺序/Strict Mode|
|13|Request module|commands → result/error|专用 module|HTTP adapter|headers/abort/stale response|
...
把散落的 presentation 条件集中为纯 module。Phase 2A 候选只提取 Candidate pool 派生计数，并由 table-driven tests 固定现有语义；Phase 2B 未执行。后续涉及 authority 的模型仍必须先覆盖完整矩阵。
