...
|5|Decision badges|Candidate presentation → JSX|容器|无|Evidence/R2.2 标签漂移|
|6|Decision View Model|pool + Task links + mode → rows|纯 module|无|authority 条件遗漏|
|7|Source import view|view model + commands → JSX|容器|无直接 fetch|preview/confirm 混淆|
|8|Candidate list/detail|rows + selection + commands → JSX|容器|portal 另行处理|DOM/菜单行为|
|9|Storage recovery module|cache adapter → hydration result|专用 Hook/module|localStorage|覆盖顺序/Strict Mode|
|10|Request module|commands → result/error|专用 module|HTTP adapter|headers/abort/stale response|
...
一次提取一个区域，不移动 state/effect/fetch。Phase 1A 完成未解锁展示叶子，Phase 1B 完成候选品池五项决策摘要，Phase 1C 完成主链路引导；Phase 1 整体仍未完成。失败直接 revert 单 Commit。
