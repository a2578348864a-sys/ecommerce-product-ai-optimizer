...
- 端口：`http://your-server-ip/api/health` = 200；`https://your-server-ip` = 超时 → **443 未放行（阿里云安全组，需控制台操作）**。
- 生产运行构建：`/path/to/project/.next/BUILD_ID = <redacted-build-id>`
  = 集成树 <redacted-hash> 构建产物（本地 `.next/BUILD_ID` 一致）→ 部署 = **预构建产物上传**，不是服务器 git 检出构建。
- 服务器 git 检出（`/path/to/project`）HEAD = `<redacted-hash>`（是 <redacted-hash> 的祖先），且有 2 个未提交修改
...
2. **证书方案**：Let's Encrypt IP 证书（IP your-server-ip）；**webroot 签发**（Certbot 5.4+，IP + webroot 模式）；
...
- **D4（部署溯源卫生）**：服务器 git 检出 ≠ 运行产物（HEAD 落后于部署提交且带未提交修改）。
  ...
  治理：见第 5 条（Phase 3 清理）。不阻断 Phase 0（运行构建已核证 = <redacted-hash> 产物）。
