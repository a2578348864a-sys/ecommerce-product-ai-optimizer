...
| `maxAiCalls/usedAiCalls`（ai_jobs_v1） | 脚本默认 **0**（`scripts/dev/create-demo-password.mjs:98`；`demoAccess.ts:284`） | 一次真实 AI 动作（12 处路由消费） | 唯一扣减 `reserveDemoAiImageCalls` → `usedAiCalls += count`（`demoAccess.ts:452`） | **无文件锁** |
