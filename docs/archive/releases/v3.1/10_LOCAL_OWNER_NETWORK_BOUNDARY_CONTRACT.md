...
- 生产 `next start -H 127.0.0.1 -p <PORT>`：<PORT> 仅绑定回环（`ss -tlnp | grep <PORT>` 确认 127.0.0.1:<PORT>）。
- nginx 唯一入口 `listen 80`，`proxy_pass` 到 127.0.0.1:<PORT>；已设 X-Real-IP / X-Forwarded-For / X-Forwarded-Proto。
