...
- 生产现状（服务器只读审计，2026-08）：PM2 `alibaba-ai-assistant` 为 **fork_mode 单实例**（pid <redacted>），
  启动参数 `next start -H 127.0.0.1 -p <PORT>`；端口 <PORT> 仅绑定 127.0.0.1（回环）。
...
- nginx 1.18.0 单 `server { listen 80; server_name _; }` 反代到 <PORT>。
...
3. `local_owner`：Visitor 码与配额行为兼容现状；继续回环绑定 <PORT>。
