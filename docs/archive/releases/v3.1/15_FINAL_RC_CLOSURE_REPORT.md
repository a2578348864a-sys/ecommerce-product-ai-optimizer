...
> 不创建 v3.1.0 标签。最终 Authority HEAD = <redacted-hash>（本文档提交后重建部署，见文末 parity 记录）。

## 1. IMAGE_REAL_ACCEPTANCE = PASS
### 1.1 前置缺陷
Golden Demo 缺 exact-product authoritative reference image：模板 creativeHandoff 的批准参考（assetFingerprint <redacted-hash>…）无对应图片资产，且 researchContext.productImage 缺失（模板无 sourceMeta、candidate sourceMetaJson 为空、且 loadCandidateSourceMeta 对 fixture 候选 id 恒返回 undefined）→ Guest Image quota=1 但真实生成永远在 Provider 前 409。
...
- 身份锁定：productKey=amazon:US:B0F2BF31PW + candidateIdentityHash/identityHash = ProductBatch facts.itemHash（<redacted-hash>…，exact item 绑定；无其他 ASIN/variant/Food Jar/10oz/Pink）。
- 加入位置（Golden Demo authority）：goldenDemoTemplateData.ts 新增 GOLDEN_DEMO_PRODUCT_IMAGE_SNAPSHOT（dataUrl+contentHash+bytes 与真实字节一致校验通过）+ 模板 resultJson 顶层 sourceMeta.candidateSnapshot.productImageSnapshot（task_snapshot 权威路径）+ 沙箱候选 sourceMetaJson（marketScreeningIdentity + 同一快照）；visualReferences[0].assetFingerprint 修正为 sha256(visual-reference:+contentHash)=<redacted-hash>…，并同步重算 handoffFingerprint（内容哈希校验第 951 行，新版 <redacted-hash>…）。
- Fresh Guest 自然获得：ensureVisitorDemoCopy 创建副本时即含 sourceMeta 与批准参考（无需人工/额外步骤）。

### 1.3 公网实测（Fresh Guest sandbox_task_demo_<redacted-hash>）
| 步骤 | 结果 | 证据 |
|---|---|---|
...
| result renders | 新草稿 <redacted-hash>-…（gpt-image-2，1536×1024 PNG 1,330,044B，needs_human_review）落盘 data/ai-image-drafts/visitor/…；imageHandoffBinding 生成（visualReferenceFingerprint=<redacted-hash>，model=openai-compatible-relay） | sandbox resultJson + 文件系统 |
...
- 修复链（main）：<redacted-hash> → <redacted-hash>（参考图资产+指纹）→ <redacted-hash>（handoffFingerprint 重算）→ <redacted-hash>（studio 响应携带 demoAccess 快照）→ 本文档提交（docs only）。
- 部署前：LOCAL_MAIN_HEAD = ORIGIN_MAIN_HEAD = PUBLIC_DEPLOYED_SOURCE_HEAD = <redacted-hash>；BUILD_ID = <redacted-build-id>；产物 v31-final.next.tar.gz（sha256 <redacted-hash>…29AE）；artifact↔SOURCE 映射：clean HEAD <redacted-hash> → npm run build（EXIT 0）→ BUILD_ID → 部署 → 健康门 200/200。
...
- 横幅实时性（发现并修复 P1，两轮）：第一轮修复 Studio 客户端（standalone 处理器）+ 两路由响应携带 buildDemoAccessSnapshot；浏览器走查（用户亲自要求）暴露真实 task-linked 流程由 ListingHandoffSection / ImageHandoffSection 组件驱动（未被第一轮覆盖）→ 横幅仍陈旧。第二轮补丁：两组件 generate 成功与拒绝分支同步 updateDemoAccessSnapshot（commit <redacted-hash>）。最终实测（Fresh Guest <redacted-hash>，全 UI 操作）：生成 Listing 后横幅实时变「Listing 剩余 0 次」，生成图片后实时变「生图剩余 0 张」，刷新后保持 0/0（配额不重置），二次生成 403 demo_standalone_image_quota_exceeded（provider 前）✓

## 4. 质量门
- 全量测试：5348→5337 passed / 3 failed = 既有基线 3 项（productUiPolish、handoff.product-journey-quota 为 <redacted-hash> 既有失败；demoSandbox.store-consistency 并行 flaky 隔离通过）+ native1688Bridge.integration 并行 flaky（隔离 11/11 通过）→ NEW_FAILURES = 0
...
- BUILD_ID = <redacted-build-id>；artifact = v31-final.next.tar.gz（sha256 <redacted-hash>）；pm2 pid <redacted>（fork 单实例，unstable_restarts=0）。
