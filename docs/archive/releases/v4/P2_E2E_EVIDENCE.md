...
- Commit：<redacted-hash>（本地 main，flag on 构建）
- 环境：http://127.0.0.1:<PORT>（local_owner；QX_V4_GRAPH_ENABLED=1；市场工具 recorded 模式）
...
- Fixture：dev.db 候选 <redacted-hash>…（THERMOS FUNTAINER，SellerSprite 导入，只读）
- 成本上限：0（零真实调用）
