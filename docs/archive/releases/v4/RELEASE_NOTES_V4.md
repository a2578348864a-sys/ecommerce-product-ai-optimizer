...
- 发布记录（2026-08-22）：validatedCodeSha=`<redacted-hash>`（lint/typecheck/全量测试 5769-0/Build/浏览器 E2E 全部实测基线）；release SHA 与 Git tag `v4.0.0` 一致；公网部署 https://your-server-ip（public_showcase，公网只读 Replay）
- 验证基线（<redacted-hash> 实测）：npm test 5769 passed / 0 failed；npm run build exit 0；浏览器：ContentHandoff 空值失败→批准参考图→生成 2 候选成功链 + Public Replay 浏览/刷新持久链
