...
- baseCommit：`<redacted-hash>`（v3.1.0，P0 已验证 clean）
...
| Owner | 路径 |
|---|---|
| Lead | docs/archive/releases/v4/*（含本契约、P1 报告）、prisma/schema.prisma、prisma/migrations/*、package.json、package-lock.json、next.config.ts、lib/v4/contracts.ts、lib/v4/featureFlag.ts、app/api/v4/*（API 契约实现） |
...
1. Lead：本契约入库（docs/archive/releases/v4/P1_CONTRACT.md）。2. Lead：prisma schema+migration+generate，跑回归。3. Lead：npm install 冻结元组 + next.config.ts，跑 lint/test/build 回归（flag off）。4. Lead：建 worktree 分支。5. Wave 1：A、B 并行实现；第 3 子 Agent 只读评审。6. Lead：逐个合并、补公共接线（API 路由）、全量门禁。7. Lead：真实浏览器 E2E + TASK_REPORT。8. Gate 判定，PASS→P2，否则停止。
