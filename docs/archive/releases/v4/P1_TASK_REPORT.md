...
- executionBatch：V4-FINAL-R2-P1-<redacted-hash>-1900；authorityChecksum：`<redacted-hash>…`
- 报告时间：2026-08-21 12:24:11 +08:00；集成 HEAD：`<redacted-hash>`（main，本地；未 push）
...
## 架构与关键决策（详见 docs/archive/releases/v4/P1_CONTRACT.md §0）
...
| commit | 内容 |
|---|---|
| <redacted-hash> | wave0：LangGraph 依赖 + Prisma schema/migration + next.config serverExternalPackages + P0 报告/P1 契约 docs |
| <redacted-hash> | 公共契约 contracts.ts（researchRun.v4）+ featureFlag.ts |
| <redacted-hash> | 合并 A(13 文件 lib/v4)+B(32 文件 app/v4+components/v4) |
| <redacted-hash> | Lead API 路由 6 个 + apiHelpers + Sidebar 接线 |
| <redacted-hash>/<redacted-hash> | API 契约测试（11 个） |
| <redacted-hash> | 集成对齐：Sidebar 渲染期门控（冻结 nav 常量保留）、ProductBatch 守卫收窄、login-journey 陈旧断言对齐（基线缺陷） |
| <redacted-hash> | E2E 集成修复：createRun draft state、checkpoint mkdir、预算统一 |
...
| 8 | 真实浏览器完成一条 fake workflow + 截图 | ✅ | 见 P1_E2E_EVIDENCE.md；截图存 `/path/to/workspace` |
...
| 浏览器 E2E（playwright-cli + <PORT> local_owner） | 全链路通过（见证据文档） |
...
- 本地 <PORT> 已恢复计划任务托管（flag off 默认），health 200。
...
- main=<redacted-hash>（本地，clean，未 push）；`git status --porcelain` 空；<PORT> 运行中（flag off）。
- 证据目录：`/path/to/workspace`（截图 + snapshot + console）。
