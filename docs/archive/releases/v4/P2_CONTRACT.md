...
- executionBatch：V4-FINAL-R2-P2-<redacted-hash>-2030；authorityChecksum：`<redacted-hash>…`
- baseCommit：`<redacted-hash>`（P1 PASS 后 main）；Lead：契约/Tool 信封/Evidence 合并/Graph 与 Report UI 接线/真实账号调用/E2E
...
| Owner | 路径 |
|---|---|
| Lead | docs/archive/releases/v4/P2_*、lib/v4/tools/envelope.ts（冻结）、lib/v4/skills/registry.ts 骨架（冻结后交 A/B 填条目？——否：registry 由 Lead 收口）、lib/v4/report.ts（Evidence→市场报告 + evidenceRefs 校验）、app/api/v4 扩展（research 报告端点）、Run Console 报告 UI 接线、Graph 集成（lib/v4/graph.ts 的 tool dispatch 改接 envelope） |
