...
- 判定：**本地发布验收通过（PASS）** —— B1/B3/B4 已于 `<redacted-hash>` 收口；剩余仅用户授权的发布动作（push / tag / deploy / README G6）
- RC SHA：`<redacted-hash>`（本地 main；**未 push/tag/deploy**——按授权边界；B1/B3/B4 收口后最终冻结）
- 报告时间：2026-08-21 16:27:30 +08:00；集成 HEAD：`<redacted-hash>`；审计收口复核 HEAD：`<redacted-hash>`（P7-A/B/C 三份只读审计全部收口后复核）
...
| build | RC `<redacted-hash>` 实测 **PASS**（停 <PORT> → npm run build exit 0 → 重启服务：health 200 / 登录页 200 / V4 flag-off 404 / `/api/opportunities` 410） |
...
| 浏览器旅程 | 全链 completed（run <redacted-hash>，rev30）+ 恢复/取消/失败路径/Replay 防篡改（409）/flag rollback |

## 发布阻断核对
| 阻断项 | 状态 |
|---|---|
| ~~B1 handoff quota 基线测试失败~~ | **已修复**（<redacted-hash>）：image-handoff gate fail-closed（409 creative_gate_unavailable）+ 回归测试；全量 0 失败 |
| ~~B3 /api/opportunities legacy 410~~ | **已修复**（<redacted-hash>）：根路由 410 + 契约测试（子路由保留，403 门禁不变） |
| ~~B4 isDemoAccessExpired 语义~~ | **已确认**（<redacted-hash>）：记录不失效契约（12h Token/Cookie 控制访问、isActive 开关、GC 独立）；注释补强，逻辑不变 |
...
- E2E 证据包：docs/archive/releases/v4/P*_E2E_EVIDENCE.md + tmp/v4-p1-evidence/（截图）+ .playwright-cli（快照）。
...
## DONE 声明格式（17）
- 完成范围：V4 P0-P6 全部门禁 + P7 本地发布验收（RC SHA <redacted-hash>，未发布；lint/typecheck/全量测试/Build 四件套全部实测通过；B1/B3/B4 收口；真实浏览器链：ContentHandoff 空值失败→批准→生成 2 候选成功 + Replay 刷新持久）。
- 测试命令与结果：见验证矩阵。
