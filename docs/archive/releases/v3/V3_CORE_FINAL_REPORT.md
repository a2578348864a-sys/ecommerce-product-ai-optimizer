...
| Phase | 结果 | 关键产出 |
|---|---|---|
| Phase 0 | PASS（含 Closeout） | 资产裁定 15 项正式风险、决策语义钉死、docs/archive/releases/v3/changes/phase-0/ |
...
- build: PASS
- local smoke: <PORT> 由 `npm run start:local` 提供；9 步 Smoke 自动化矩阵 PASS + 人工页面步骤文档化（validation.md §3）
...
- <PORT> restore: 不适用（未停计划任务）

## Git

- main: `<redacted-hash>`
- origin/main: `<redacted-hash>`（本地领先，未 push）
...
3. **人工验收执行记录**：用户按 manual-smoke-checklist.md 在 <PORT> 执行全部 9 步，结论 MANUAL_CORE_SMOKE = PASS；验收排障期间修复 5 个真实 bug（Evidence Workbench 挂载、DTO 投影、认证头、关键词 schema、AI 总结 schema/重试），全部经测试 + 页面实测
