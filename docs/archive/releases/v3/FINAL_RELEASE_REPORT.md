...
> Git HEAD：`<redacted-hash>`（main == origin/main，working tree clean）
> 公网部署：https://your-server-ip（V3 Public Demo，Visitor 沙箱隔离）
...
> 完成 F1-F11 全部整改（P1=0）→ 重新批准 → V3 公网部署（your-server-ip）→ V3 冻结（<redacted-hash>）。
...
1. 由本机计划任务 `local-runtime-task` 自动管理（`npm run autostart:local` 注册）。
2. 打开 `http://localhost:<PORT>`，输入管理员密码，或使用访客码体验。
3. 停止/重启：`npm run start:local` / `npm run check:local`（禁止替代 <PORT> 的其他端口入口）。
...
- HEAD：`<redacted-hash>`（v3 final product integration：F1-F11 全部 FIXED，P1=0）
- main == origin/main == <redacted-hash>；tracked clean。
