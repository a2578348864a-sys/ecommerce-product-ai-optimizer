...
| LAST_CODE_HEAD | <redacted-hash>（feat: evidence-to-creative context bridge，V3 最终产品代码） |
| <PORT> runtime | production `next start`（127.0.0.1:<PORT>），health OK，runtime 含 <redacted-hash> 代码（字段级实证） |
| planned task | local-runtime-task = Ready |
| Prisma DB | prisma/dev.db（SQLite，11 tasks / 11 candidates，quick_check=ok，审计备份留存） |
| Release artifact | `node scripts/release/package-release.mjs` → `release/next-v2.2.16-<sha>-linux-x64.tar.gz`（本轮已实证打包成功） |
...
| native1688Bridge.integration.test.ts | ENVIRONMENT_SKIP | "bridge did not start"：<PORT> 端口被运行中 <PORT> 的 bridge 占用；端口常量硬编码（无 env 覆盖），无法独立端口；已有真实 bridge runtime evidence（toolStatus loggedIn=true / extension 0.3.1 / versionCompatible=true） |

**本轮发现并修复的真实 release bug**：`scripts/release/package-release.mjs` 在中文路径 Windows 工作区无法产出 artifact（Node→Windows bsdtar 参数编码损坏：输出路径 `/d/…中文…` 无法打开）。修复：artifact 先在 ASCII 临时目录打包校验，再用 Node fs 复制到最终目录；tar 参数使用 Windows 原生路径；`rm` 命令改用 Node fs。修复后真实打包 PASS（manifest 含 BUILD_ID/SHA256），测试适配同步。Linux 服务器路径（`/path/to/project`）不受影响。

## 5. Remote Sync

| 项 | 状态 |
|----|------|
| LOCAL_MAIN | <redacted-hash>（含 Final Freeze Audit + hygiene + tooling fix + 本文档） |
| REMOTE_MAIN（真实远端） | <redacted-hash>（`git ls-remote` 实证） |
| AHEAD/BEHIND/DIVERGED | AHEAD=24 / BEHIND=0 / 纯领先（<redacted-hash> 为本地祖先，fast-forward 验证通过） |
| 传输 | SSH 22 可达（`GIT_SSH_COMMAND="ssh -o ConnectTimeout=15 -o BatchMode=yes"`）；push 成功 `<redacted-hash>..<redacted-hash> main -> main` |
| Push 后实证 | REMOTE_MAIN_HEAD == LOCAL_MAIN_HEAD（<redacted-hash>）；`git status -sb` 无 ahead/behind |
...
- 完整研究链路（采集→证据→AI→决定→完成→创作），依赖本地：浏览器（Amazon 页面采集）、1688 CLI 登录态、Qingxuan 1688 Helper 扩展（0.3.1）+ Authenticated Loopback Bridge（127.0.0.1:<PORT>-<PORT>）。
...
- 现状：`prisma/dev.db`（SQLite，本地）。公网服务器已有独立运行数据（`/path/to/project` 部署历史）。
- 部署方式：本地 `npm run build` → `node scripts/release/package-release.mjs` → artifact **只含 `.next/`**（不含 dev.db/.env/node_modules）→ 服务器解压替换 `.next`。
- **Release 不会覆盖公网 DB**：artifact 不含数据库文件；runbook 明确禁止上传本地数据库；`prisma/dev.db` 在 `.gitignore`，永不进入 artifact/仓库。
- 生产 DB 位置：持久化目录（建议独立于 release 目录，如 `/path/to/project/data/` 或显式 DATABASE_URL 指向的卷），首次部署 `prisma migrate deploy`（无 schema 变化时跳过），后续 `db:backup`（`scripts/ops/db/protect-sqlite-db.mjs`）定时备份。
...
- 现状（runbook）：公网 IP your-server-ip；Nginx 反代 → `127.0.0.1:<PORT>`；PM2 服务名 `alibaba-ai-assistant`；<PORT> 仅监听 loopback。
- 部署计划检查点（部署阶段执行，本轮不修改）：
  - domain/IP + port：沿用现有配置或按用户新域名调整
  - proxy target：127.0.0.1:<PORT>
...
- 进程：PM2 管理，`pm2 restart alibaba-ai-assistant`；启动 `next start -H 127.0.0.1 -p <PORT>`。
...
| DB_ROLLBACK | 与代码回滚**分离**：`scripts/ops/db/protect-sqlite-db.mjs`（backup/predeploy/postdeploy）+ 服务器 DB 文件备份；回滚 DB 不触碰代码目录 |
...
| 项 | 值 |
|----|----|
| 部署对象 | your-server-ip（iZbp1cvdvghmferp57a3tzZ）/ `/path/to/project` / pm2 `alibaba-ai-assistant` |
| Artifact | `release/next-v2.2.16-85d5454-linux-x64.tar.gz`（1,779,831 bytes） |
| Artifact SHA256 | `<redacted-hash>` |
| BUILD_ID | `<redacted-build-id>`（与本地 main <redacted-hash> 构建一致） |
| 部署前服务器状态 | 旧版运行中（BUILD_ID <redacted-build-id>，uptime 5D）；服务器 git 存在 5 个 listing 模块未提交修改——已比对确认其内容与本地 main 一致（3 文件仅 BOM 差异；2 文件为本地 main 的 V3 creativeContext 超集），替换不丢失任何服务器独有逻辑 |
...
| PM2 | restart 成功（PID <redacted>，status online，restart count 81） |
| 健康检查 | 服务器本地 `127.0.0.1:<PORT>/api/health` = `{"ok":true}`；公网 `/api/health` = 200 |
