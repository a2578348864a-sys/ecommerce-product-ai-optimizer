...
- 审计基线：`main` HEAD = `<redacted-hash>`（docs：evidence-to-creative context closure）；最后一个代码提交 = `<redacted-hash>`（feat：evidence-to-creative context bridge (P1)，2026-08-17 23:02:57 +0800）
- 运行实例：<PORT>（`next start -H 127.0.0.1 -p <PORT>`，production 模式，PID <redacted>，health OK）
- 数据库：`prisma/dev.db`（SQLite，11 tasks / 11 candidates，`quick_check=ok`；审计前备份 `.local-backups/db-guard/2026-08-17T23-15-34`，SHA256 `<redacted-hash>`）
- 纪律：只取证/定位/最小复现/记录；未修改任何业务代码、未执行 DB 迁移、未调用付费 Provider、未做公网部署；<PORT> 保持运行。
...
V3_6                   = NOT_AUTHORIZED
REMOTE_SYNC            = BEHIND          (本地 main 领先 origin/main=<redacted-hash> 共 5 提交：<redacted-hash>/<redacted-hash>/<redacted-hash> + 审计报告 2 提交，未授权 push)
...
| 8 | 公网能力边界 | **明确** | Bridge/1688 CLI/浏览器扩展全部本地 loopback（127.0.0.1:<PORT>-<PORT>），公网实例无法访问用户 Windows 资源；服务端 fetch 面仅 127.0.0.1（无任意 URL fetch，无 SSRF/open proxy 面） |
...
| 双真相检查 | resultJsonHash（storageVersion） | 各端点返回一致 hash `<redacted-hash>…948b2`（4dauumii） |
...
| Bridge 绑定 | loopback only | 127.0.0.1:<PORT>-<PORT>，256bit token 内存持有，job 绑定 taskId/candidateId/imageHash | PASS |
...
| V3F-02 | P2 | 任务 nmi837j6（Owala FreeSip VVD）为孤儿任务：resultJson 中 candidateId=<redacted-hash>… 已不在 opportunityCandidate 表 | 候选被删除后任务保留（历史数据状态） | 研究记录/历史详情（数据卫生） | 对比 /api/tasks 与 /api/opportunity-candidates | 11 tasks vs 10 candidates；browser-evidence 返回 candidateId=<redacted-hash> 无对应候选 | 可选：清理或保留并标注 | 低 |
| V3F-03 | P2 | `data/demo-product-batches/demo_<redacted-hash>.json`（1.7MB sandbox 数据）不被 .gitignore 覆盖 | gitignore 仅覆盖 demo-access.json / demo-sandbox.json | git add -A 有误提交风险 | `git check-ignore -v` 无输出 | 未跟踪，但 git status 显示 `??` | 将 data/demo-product-batches/ 加入 .gitignore | 极低 |
...
| RESTART_SMOKE（<PORT> 重启冒烟） | NOT_EXECUTED | 中断性操作未授权；当前进程健康（health OK）且 runtime=main 已实证 |
| Full test suite / tsc / lint / build | 未全量重跑 | 核心 6 文件 209/209 PASS（本次会话重跑）；known failures 本次会话重新确认（见下） |
| Known failures 重新确认 | 完成 | `native1688Bridge.integration.test.ts`：fail "bridge did not start"（<PORT> 端口被运行中 <PORT> 的 bridge 占用），11 tests skipped —— 环境性失败，与代码无关；`scripts/release/release-package.test.ts`：fail execFileSync（Windows tar 基线差异），3 tests skipped —— 环境性失败，与代码无关 |
| Planned task | Ready | `schtasks /Query \local-runtime-task` = Ready；127.0.0.1:<PORT> 由 PID <redacted> 监听 |
...
| REMOTE_SYNC（push 本地 main） | BEHIND | origin/main=<redacted-hash>（HTTPS ls-remote 实证可达）；本地领先 5 提交（<redacted-hash>/<redacted-hash>/<redacted-hash> + 审计报告 <redacted-hash>/<redacted-hash>）；SSH 22 不可达；push 未授权，未改 remote 配置 |
...
- 本报告为唯一审计产物；未修改任何业务代码/数据；<PORT> 保持运行。
...
| FINAL_FULL_REGRESSION | PASS | vitest 4932 passed / 90 skipped；TSC exit 0；LINT 0 errors；BUILD exit 0；release-package.test PR-1 PASS（VR 系列 ENVIRONMENT_SKIP：Windows 无 bash）；native1688Bridge.integration ENVIRONMENT_SKIP（<PORT> 端口被 <PORT> bridge 占用，端口硬编码无 env 覆盖，已有真实 bridge evidence） |
| RELEASE_TOOLING_FIX | 完成（<redacted-hash>） | package-release.mjs 中文路径 Windows 工作区打包失败（Node→bsdtar 输出路径编码损坏）为真实 release bug；修复：ASCII 临时目录打包+校验+Node fs 复制；修复后真实打包 PASS（release/next-v2.2.16-<redacted-hash>-linux-x64.tar.gz 1,779,011 bytes，manifest SHA256 记录） |
| RESTART_SMOKE | NOT_EXECUTED | 无业务代码变化（仅 .gitignore/docs/release-tooling），无需重建 <PORT>；中断性操作未授权；<PORT> 持续 health OK |
| REMOTE_SYNC | PASS | 本地 <redacted-hash> = 远端 <redacted-hash>（ls-remote 实证）；AHEAD=24/BEHIND=0 纯领先 fast-forward push（SSH 22 可达，BatchMode）成功 `<redacted-hash>..<redacted-hash>`；无 force/rewrite；`git status -sb` 无 ahead/behind |
| PUBLIC_PREFLIGHT | 见 docs/archive/releases/v3/PUBLIC_DEPLOYMENT_READINESS.md | 17 节只读规划；PUBLIC_PREFLIGHT_BLOCKERS=0；PUBLIC_DEPLOY=FORBIDDEN |
