...
> 来源：唯一权威合同 `docs/archive/releases/v3/V3_5_PRE_IMPLEMENTATION_CONTRACT.md`；本目录各分项文档。
> 分支：`codex/v3-5-implementation`（worktree `电商工具-v3-5-impl`）；起点 <redacted-hash>。
...
| 43 | docs path | `docs/archive/releases/v3/changes/v3-5-implementation/`（proposal / architecture-decision / keyword-acquisition / image-acquisition / sourcing-evidence / browser-security / real-smoke / security-review / regression 并入 security-review / learnings 并入 browser-security） |
| 44 | commits | 8 个本地 commit（<redacted-hash> → <redacted-hash>），已全部并入 main 并 push |
| 45-47 | branch / main / origin main | branch `codex/v3-5-implementation`（已完成交付）；**main == origin/main == <redacted-hash>** |
| 48 | push status | **已 push**（2026-08-16 验收后 fast-forward 集成 main，<redacted-hash>..<redacted-hash>） |
...
```
<redacted-hash> feat: degrade gracefully when detail enrichment is risk-controlled
<redacted-hash> docs: record acceptance findings (real keyword smoke PASS + 2 real bug fixes; ...)
1579918 fix: bind page session for Page/DOM/Input CDP commands
<redacted-hash> fix: raise SKU cap to 500 for real multi-spec offers
<redacted-hash> test: fix sandbox task input shape and resolver fixture completeness
<redacted-hash> test/docs: close v3.5 implementation (docs + risk-control envelope mapping)
<redacted-hash> feat: add sourcing evidence panel to task detail (preview -> human confirm)
<redacted-hash> feat: add native 1688 image acquisition (persistent browser session + versioned resolvers)
<redacted-hash> feat: add sourcing evidence workflow (preview -> human confirm -> save)
<redacted-hash> feat: add v3.5 sourcing acquisition contracts and read-only CLI driver
```
...
V3_5_REMOTE_CLOSEOUT        = PASS（fast-forward 集成 main + push；origin/main == main == <redacted-hash>）
...
main == origin/main         = <redacted-hash>
...
|---|---|---|
| <redacted-hash> | offer 674035283676 有 128 SKU（颜色×容量），SKU 上限 100 误拒绝 | 上限放宽 500（仅防无限膨胀） |
| <redacted-hash> | `PersistentBrowserSession.send` 未绑定 page session，Page/DOM/Input 域命令 -32601 | send 默认 page session + sendRoot 显式浏览器域 |
| <redacted-hash> | 1688 风控恢复窗口内第二次调用必 challenge，save 被 enrich 阻断 | enrich 降级：失败记录 platform_metadata 标记并继续落盘（§69 revalidate 对象是 preview 候选） |
