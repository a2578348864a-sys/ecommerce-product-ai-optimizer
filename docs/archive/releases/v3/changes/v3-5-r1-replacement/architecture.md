...
> 基线：main == origin/main == <redacted-hash>；分支 codex/v3-5-r1-formal-replacement
> 权威修订：`docs/archive/releases/v3/V3_5_PRE_IMPLEMENTATION_CONTRACT.md` §42（R1 Amendment）
...
ImageAcquisitionDriver
  ...
  ↓ Authenticated Loopback Bridge（extensions/qingxuan-1688-helper/bridge/server.mjs；127.0.0.1:<PORT>）
...
1. docs: amend v3.5 image acquisition contract（<redacted-hash>）
2. feat: add no-debugger 1688 image extension + authenticated loopback bridge（<redacted-hash>）
3. feat: replace v3.5 image acquisition driver（<redacted-hash>）
4. test: validate no-debugger image acquisition（<redacted-hash>）
5. test: extension source invariant audit（<redacted-hash>）
