...
> 基线：main == origin/main == <redacted-hash>（PLAYWRIGHT_FINAL_ACCEPTANCE 被用户手工反例 INVALIDATED；LOCAL_RELEASE_CANDIDATE=REVOKED）
...
2. `<redacted-hash>` fix(workbench): AI summary 唯一动作 + Decision 显式保存（R2/R3）
3. `<redacted-hash>` fix(studio): 修复 research studio handoff 解析 + 停止假 CTA（R4/R6）
4. `<redacted-hash>` fix(workbench): 研究资料状态由已确认 Evidence 派生并自动同步（R7）
5. `<redacted-hash>` feat(nav): 拆分商品研究/研究记录（R5）
