...
- 实测：`dns.resolve4` → `ECONNREFUSED`；`dns.lookup`（与实际 HTTP client 同路径）→ 返回代理 fake-ip `your-server-ip-128`；`nslookup` Server 指向代理虚拟 DNS `your-server-ip`。
- **PROXY_FAKE_IP = 确认**（代理把公网域名解析到 your-server-ip/8 保留段；your-server-ip/15 为同类常见 fake-ip 网段）。
...
- **fake-ip 识别**：域名解析结果落在 your-server-ip/8、your-server-ip/15 → 标记 PROXY_FAKE_IP 并放行（请求实际经代理按域名路由，不会发往内网）；
- **保留全部禁令**：localhost/your-server-ip/8/::1、RFC1918、link-local（含 metadata your-server-ip）、CGNAT、private/reserved IPv6、file/ftp/data/javascript、userinfo、非 443 端口、解析失败 → 全部 fail-closed；
- **IP 字面量保留段输入仍然拒绝**（用户直接输入 your-server-ip / your-server-ip 等 → literal_private，不因 fake-ip 放行字面量）；
...
| 为什么不可用 | **Case A 变体：V35_1688_CLI_PATH 从未配置**（`.env.local` 无此键；<PORT> 进程无该 env）——工具存在、登录有效，仅缺配置/发现 |
...
- 全量：**4872 passed / 90 skipped**；仅 2 个**既有环境失败**（native1688Bridge <PORT> 端口被运行中 <PORT> 占用；release-package Windows tar 基线——main@<redacted-hash> 已存在，非本轮引入）。
- tsc PASS；lint 0 errors（14 既有 warnings）；build PASS。
- <PORT> 运行最终构建（BUILD_ID `<redacted-build-id>`），health 200，计划任务原状态。
...
> 2026-08-17 · 分支 `codex/v3-final-runtime-closeout-r13` → main（<redacted-hash>）
...
- 全量：**4880 passed / 90 skipped**（+8 新用例）；仅 2 个既有环境失败（bridge <PORT> 端口被运行中 <PORT> 占用；release-package Windows tar 基线）。
- tsc PASS；lint 0 errors；build PASS；R1-R12 无回归。
- <PORT> 运行新构建（health 200，计划任务原状态）。

## 6. 版本清单（§215）

- APP_BUILD_HEAD：`<redacted-hash>`（R13 修复）
...
> 2026-08-17 · 分支 `codex/v3-1688-candidate-images` → main（<redacted-hash>）
...
| HTTP_STATUS | 直接访问 **200**；**带 Referer: http://127.0.0.1:<PORT>/ → 403** |
| CONTENT_TYPE | image/jpeg（150-220KB 真实图片） |

**定性 = E 类：1688 CDN（alicdn）防盗链**——浏览器 img 请求携带页面 Referer（127.0.0.1:<PORT>）→ alicdn 403 → broken。非 Next/Image（普通 img）、非 normalization、非代理、非 cookie、非 URL 缺失。
...
## 3. 真实验收（headed + 真实 <PORT>，§21/§22/§23）
...
after 截图：`docs/archive/evidence/releases/v3/changes/v3-final-runtime-closeout/r14-image-search-after.png`（§30；before 状态以修复前 10/10 broken 的 DOM 统计为证据）。

## 4. 测试与质量

- 新增 7 用例：normalizeCandidateImageUrl（4 类拒绝 + protocol-relative + keyword 链集成）、SourcingCandidateThumb（no-referrer 渲染 + 占位 fallback）。
- 全量：**4887 passed / 90 skipped**（+7）；仅 2 个既有环境失败（bridge <PORT> 端口占用、release-package tar 基线）。tsc / lint（0 errors）/ build PASS；R1-R13 无回归。
- <PORT> 运行新构建（health 200，计划任务原状态）。
...
> 2026-08-17 · 分支 `codex/v3-remove-legacy-research` → main（<redacted-hash>）
...
- 更新 ProductResearchDecisionPanel.test.ts（legacy 分支移除断言 + 详情页只读卡断言）；全量 **4887 passed / 90 skipped**（仅 2 个既有环境失败：bridge <PORT> 端口占用、release-package tar 基线）；tsc / lint（0 errors）/ build PASS。
- Git：独立分支 `codex/v3-remove-legacy-research`（<redacted-hash>）→ main；备份不提交。
- <PORT> 运行新构建（health 200，计划任务 Ready/Enabled）。
...
> 2026-08-17 · 分支 `codex/v3-current-research-normalization` → main（5 commits：<redacted-hash> / <redacted-hash> / <redacted-hash> / <redacted-hash> / <redacted-hash>）
...
- `lib/server/productResearchRecordStore.ts`：
  - `getProductResearchDecisionState`：无 record → `{legacy:`<redacted>`
...
## 3. 真实验收（headed + 真实 <PORT>，THERMOS cmsw7363z0002cih40bujcawy）
...
- **FULL_REGRESSION = PASS**：`npx vitest run` → **4907 passed / 90 skipped**（仅 2 个既有环境失败：native1688Bridge <PORT> 端口被 <PORT> 占用、release-package Windows tar 基线——均非本轮引入）。
...
- 分支 `codex/v3-current-research-normalization`（5 commits）→ main（ff-only）：
  - <redacted-hash> fix(v3): normalize current research lifecycle - candidate_research is CURRENT_ACTIVE
  - <redacted-hash> fix(v3): restore unified human decision editing for all current research
  - <redacted-hash> feat(v3): close research via POST /api/tasks/[id]/complete
  - 8835047 fix(v3): completion control reads projected summary - real headed acceptance find
  - <redacted-hash> test(v3): visitor fixture journeys for research completion gates
- <PORT> 运行最新构建（health 200，计划任务 local-runtime-task Ready/Enabled）。
- DB 轻量备份：`.local-backups/db-guard/2026-08-17T15-31-17/dev.db`（SHA256 `<redacted-hash>`，备份于真实 Journey 之前；不提交）。
...
- <PORT> 保持运行，等待用户亲自验证 Journey（商品研究 → 保存决定 → 完成研究 → 研究记录）。

---

# V3 Human Decision Authority Consistency Fix — P1 最终报告

> 2026-08-17 · 分支 `codex/v3-human-decision-authority`（<redacted-hash>）→ main
...
- **targeted：150 passed**（13 files）；**full regression：4915 passed / 90 skipped**（仅 2 个既有环境失败：native1688Bridge <PORT> 端口占用、release-package Windows tar 基线，非本轮引入）；tsc / lint（0 errors）/ build 全 PASS。
- **headed 真 <PORT>**：Bentgo 全流程（初始徽标 → 表单 → 保存 → 顶部同步 → F5 → 列表标签）实测通过。
...
- 分支 `codex/v3-human-decision-authority`（<redacted-hash>，12 files +319/-26）→ main（ff-only）。
- <PORT> 运行修复后构建（health 200，计划任务 Ready/Enabled）。
...
- 全量回归：**4932 passed / 90 skipped**（仅 2 个既有环境失败：native1688Bridge <PORT> 端口占用、release-package Windows tar 基线，非本轮引入）

## 75-76. Bentgo Headed Journey（真实 <PORT>）
...
- DB 备份（headed Journey 前）：`.local-backups/db-guard/2026-08-17T22-51-57/dev.db`（SHA256 <redacted-hash>，不提交）
- tsc / lint（0 errors）/ build 全 PASS；targeted + full regression 通过；headed Playwright 真 <PORT> 全流程
...
- **STOP**：未继续 V3.6/公网部署/RAG/1688/Amazon/VOC/Keyword 采集扩展/UI polish；<PORT> 保持运行（health 200），等待用户亲自验证"商品研究收集的资料现在 Listing/Image 有没有真的用上"。