...
> 基线：main `<redacted-hash>`（Phase 0 开始前 clean）
...
- 只读盘点：不修改任何业务代码、不迁移数据、不启动本地服务、不触碰 <PORT> / prisma/dev.db / demo 数据文件。
- 文档产物：仅写入 `docs/archive/releases/v3/changes/phase-0/`（5 个文档，见 §4）。
...
`docs/archive/releases/v3/changes/phase-0/`：
