...
> 基线：main == origin/main == <redacted-hash>（上一任务 LOCAL_RELEASE_CANDIDATE 重新 APPROVED 后撤回）
> 权威审计：`docs/archive/releases/v3/V3_FINAL_USER_OPERABILITY_AUDIT.md`（P0=0，P1=3：OA6/OA7（Amazon 生产 100% 失败+泄漏）、OA4（1688 onboarding）、OA1（Active Research 入口）；P2=4：OA2/OA3/OA5/预填）
...
- 真实普通用户 Journey（<PORT>）逐任务走查：找商品→加入研究→找到正在研究→收集 Evidence→Amazon 采集→VOC→1688 找货→AI 整理→Missing→人工决定→Listing/Image
...
- main 集成（fetch 确认 <redacted-hash> → ff）+ push + <PORT> 切换最终 build
