...
> 环境：集成树 <PORT> 生产 build（main == <redacted-hash>）；真实浏览器 playwright-cli 走查；真实 DeepSeek（real mode）；真实 Amazon/1688 网络。
...
| # | 用户任务 | 操作 | 结果 |
|---|---|---|---|
| 1 | 进入工作台 | 打开 <PORT> → 输入访问密码 → 进入 | ✅ 首页正常（16 候选 / 7 研究记录 / 五步流程卡） |
