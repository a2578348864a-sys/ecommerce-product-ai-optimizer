...
- **AUTH / SEARCH / ITEM / CURRENT-TAB / STORE / 重启复测**：⏳ **NOT_TESTED——待用户完成隔离 profile 准备**（USER_ACTION_REQUIRED 已发出，2026-08-15；连续多轮未完成）。
  - 用户待办：双击 `/path/to/workspace`（隔离 profile `/path/to/workspace` + Load unpacked `…\OpenCLI\extension`）→ 新窗口**只登录 1688** → 打开搜索结果页 → 保持窗口。
...
- **准备路径**：`launch-test-chrome.bat`（隔离 profile `/path/to/workspace` + `--load-extension`）。**实测发现：Chrome 151 忽略命令行 `--load-extension`，扩展未自动加载**（daemon 显示 disconnected；profile 无扩展）→ 用户需手动 `chrome://extensions` → 开发者模式 → Load unpacked `…\OpenCLI\extension`。**首装摩擦记录：命令行加载不可靠，必须手动加载（新手门槛+1）**。
