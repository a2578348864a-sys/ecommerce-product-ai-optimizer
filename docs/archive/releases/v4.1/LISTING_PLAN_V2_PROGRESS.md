...
## 任务0：冻结与诊断（≤10行）
- 基线：branch=feature/v4.1-ui-productization；HEAD=<redacted-hash>…；staged=0；dirty -uall=68；<PORT> pid=<redacted> health=200；dev.db SHA=<redacted-hash>… mtime=<redacted-time>（只读记录）。
...
- **真实浏览器闭环（<PORT> 隔离 v2 db，Provider 关闭，普通 Chrome/CDP）**：
...
- **状态**：HEAD=<redacted-hash>（未变）；staged 空；dirty 77（68 既有 + 9 白名单新文件；无白名单外新增）；<PORT> 未停；原 dev.db 未写（仅隔离 v2 副本）。
...
- **状态**：HEAD=<redacted-hash>（未变）；staged 空；dirty 77（68 既有 + 9 白名单，无新增）；<PORT> 已停；<PORT> 未动；原 dev.db 未写；0 commit/push/deploy。
...
- **套件**：8 文件核心 92/92；listingHandoff+server+DOM 34 文件 541/541（>489，含新增用例，0 新增 skip）；tsc 0；eslint（6 文件）0；git diff --check clean；npm run build OK（BUILD_ID <redacted-build-id>，bundle 含新 UI 串）。
- **真实 <PORT> 隔离浏览器闭环（独立服务，Provider 空 key + 死端口 <PORT>；普通 Chrome/CDP）**：
  - 隔离实例：Desktop/qingxuan-smoke/v2fix2-listingplan-2026-08-26（dev.db 副本 + demo-access 副本；原 dev.db 未写）；启动：node_modules/next/dist/bin/next start -H 127.0.0.1 -p <PORT> + DATABASE_URL/DEMO_SANDBOX_STORE_PATH/DEMO_ACCESS_STORE_PATH/QX_RUNTIME_MODE=local_owner；PID <redacted>；health {"ok":true}；隔离库 dev.db mtime 21:24:10.834Z → 21:24:36.990Z（写入）。
  ...
  - 结束：<PORT> 已停（kill + port free）；<PORT> PID <redacted> health 200 未动；原 dev.db SHA <redacted-hash>/mtime <redacted-time> 未变；data/demo-sandbox.json 未变。
- **状态**：HEAD=<redacted-hash>（未变）；staged 空；dirty 81（68 既有 + 13 候选；本轮仅白名单 6 文件，无新文件）；0 commit/push/deploy；0 原库写入；0 付费调用（Provider 空 key 死端口，providerSucceeded=false 证实未成功调用）。