...
- **浏览器验证门禁说明**：<PORT>（旧启动）前端密码门（无明文密码且禁止改 data/demo-access.json）→ 浏览器验证改用隔离 <PORT>（local_owner 免密 + 隔离库；stok demo token 由 ACCESS_PASSWORD 派生密钥离线铸造，未改动任何环境文件）。
...
记录：
- 全量 npm run test 一次：3 个文件级失败——① lib/server/native1688Bridge.integration.ts（B-001 历史间歇，隔离 11/11 通过 4.18s（前轮记录））；② scripts/dev/local-smoke-runtime.test.ts（隔离复跑 **13/13 通过 1.55s**——全量并行负载冲突，非本轮功能失败）；③ tools/upstream/generate-stage15-source-native-result.test.ts（负载型 5s 超时，既往隔离通过）。按规矩仅隔离首失败一次（②，因可能本轮相关）；①③ 已有隔离通过记录，未再循环。
...
- <PORT>/原始 dev.db：为最终只读哈希校验短暂停止 <PORT>（停前未收到任何写请求；DEVDB_MATCHES=True）；HEAD 未变；隔离实例 <PORT> 已停止并保留：/path/to/user SHA-256=<redacted>…证据保留，未删除）。
---
...
2. **dev.db 哈希（记录，非本轮造成）**：轮 12 中实测原始 prisma/dev.db SHA-256 = <redacted-hash>，文件 mtime <redacted-time>；与轮 11 归档的 <redacted-hash> 不一致。写入时刻早于本轮全部交互验收（本轮验收自 03:26 起、全程 GET/只读 + 会话标记注入，未向 <PORT> 发任何写请求；全量测试与验收均未触碰 dev.db）；dirty 集合同轮 12 任务 0 快照一致，无白名单外改动。按规矩无法回滚（无原哈希备份），如实记录；建议领导知悉后由有权限方核对 02:20 时段操作（该时刻 <PORT> 由 QingXuanAgent 计划任务或用户操作运行中）。

3. **native1688Bridge 全量失败根因（已复原）**：全量一次中 2 失败之一为端口 <PORT> 被遗留桥进程（PID <redacted>，server.mjs --token…）占用导致「bridge did not start」——非本轮代码问题；清理后隔离 11/11 通过 3.98s。demoSandbox.store-consistency 全量 5s 超时、隔离 9/9 通过 3.18s（负载 flake）。
...
1. 写入时刻：dev.db mtime <redacted-time>、大小 <redacted-hash>；SHA-256 <redacted-hash>（轮 12 记录 <redacted-hash>…、大小 <redacted-hash>——本轮又变化一次）。
...
3. 排除项（均实测）：① <PORT> 上 GET /api/tasks、GET /api/tasks/[id] 前后哈希不变；② 本轮所有 vitest 文件（8 基线 2 次、新测试 2 文件、10 文件组合）前后哈希不变；③ 90 秒零活动监控哈希不变；④ 唯一服务进程=<PORT> next start（PID <redacted>+5216+28316，同组创建于 19:25:22-23Z，无 <PORT>/其它实例）；⑤ 计划任务 local-runtime-task 最后运行 13:19:47Z（与本写入无关）。
...
5. 未做：未 stop <PORT>（无需）；未复制/编辑 dev.db；未回滚；PG SQLite 只读查询经由运行中服务 API，未写。


## 轮 13 全量失败归类（已完成隔离复跑）

- 全量一次 3 files / 4 tests failed：native1688Bridge 桥端口并发占用（隔离 11/11 绿）、stage15-source-native-result（隔离复跑进同一组 25/25 绿）、stage15-source-native-effectiveness（同上）。根因：并行 vitest 负载 + 桥进程端口固定常量 <PORT> 无法并发。非轮 13 功能失败。
...
2. **native1688Bridge.integration 隔离仍失败（B-001 环境边界，非本轮代码）**：`node scripts/dev/local-next-runtime.mjs` 进程（PID <redacted>，<PORT>）派生的真实桥 `extensions/qingxuan-1688-helper/bridge/server.mjs --token …`（PID <redacted>）一直占用 TCP <PORT> LISTENING（且 CLOSE_WAIT/FIN_WAIT_2 残留连接）；integration 测试需要自己绑定该端口做 bridge 冒烟 → `bridge did not start`。**这是本机服务常驻与集成测试的固有冲突（轮 13 B-001 同根因），需要 <PORT> 完全停止才能通过；不能杀真实 bridge 进程（属 <PORT> 服务）**。全量 2 个失败计数即：demoSandbox.store-consistency（5s 超时，隔离 11/11 通过）+ mutationBoundary（5s 超时，隔离 11/11 通过），均为负载型 flake。
3. **dev.db 读取限制**：`certutil -hashfile` 对 prisma/dev.db 报 ERROR_SHARING_VIOLATION（<PORT> 独占）；改用 Node 原生 openSync 只读句柄计算 SHA-256 成功 = <redacted-hash>（与轮 13 存档一致；mtime <redacted-time> 未变）。
4. **<PORT> 服务进程在 build 前后保持（无重启需要）**：next start 每请求从 .next 磁盘读产物；`npm run build` 替换 .next 后，运行中的 <PORT> 直接服务新构建，无需重启（已验证 health 200 / runtime-mode 正确 / tasks 页面 200）。
...
2. **dev.db 哈希变化（记录，非本轮造成）**：原始 prisma/dev.db SHA-256 从轮 13 存档 <redacted-hash> 变为 <redacted-hash>（size <redacted-hash>，mtime <redacted-time>）。写入时刻早于本轮全部操作（本轮 12:40+ 起，全部写走 <PORT>/<PORT> 隔离库；<PORT> Etekcity 任务 updatedAt 仍是 2026-08-19 未变——证明未写）。属于轮 14 期间（04:38）的既有环境写入（<PORT> 计划任务/用户操作），本轮未写原库。
3. **native1688Bridge.integration（B-001 环境边界，同轮 13/14）**：<PORT> 真实桥（PID 派生自 <PORT>）占用 TCP <PORT> → integration 测试无法自起桥 → 全量失败、隔离 11 skipped。需要 <PORT> 完全停止才能通过；不杀真实桥。全量本次 1 文件失败（此为唯一失败），与轮 14 一致。
...
4. **BUILD_ID 差异**：任务书基线 6sGWmfbpSVfny989HxClK 为轮 15 中途 build；实测轮 15 末 BUILD_ID <redacted-build-id>（确认对话框修复），轮 16 末 SlILVvNd7YLMxuA9rmXld。dirty 233（任务书 228 → +5：r16 新增 autoKeywordPlan/claimTier/测试/document）。GIT HEAD <redacted-hash> 不变。
