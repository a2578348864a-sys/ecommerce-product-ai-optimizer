...
> 审计人：本会话 Agent（只读）；日期 2026-08-23。范围：当前工作树（HEAD <redacted-hash> + Formal v2 未提交改动，BUILD_ID <redacted-build-id>）。**本轮不部署公网**。依据按实际代码/契约/实测，不引用历史文档目标当实现。
...
4. **最小部署顺序与回滚点**：本地 build（已产出 BUILD_ID <redacted-build-id>）→ package-release.mjs → scp artifact → 服务器备份 .next → 解压 → pm2 restart → health 200 → 公网 smoke（/、/api/health、guest 铸造、首页渲染）→ 日志观察 24h。回滚：恢复 .next.bak-<ts> + pm2 restart（DB 不触碰）。
...
- 未调用任何外部/公网/真实 AI/外部采集；未修改任何 DB/权限/配额/prisma/schema/依赖/配置/.env*；未 Git 写操作；未部署。公网矩阵与审计全部基于本机工作树只读核对 + 本机 <PORT> local_owner 实例的只读实测（health/runtime-mode/tasks/detail 200；GET 前后 dev.db 哈希不变）。
