...
### 隔离<PORT>浏览器验收（复制库 dev-lfc.db + Provider空key + 死端口，0付费调用）
...
- console 0 error/0 warning；外部请求 0（仅 127.0.0.1:<PORT>）✓
- 截图：<PORT>-unqualified-1440.png / <PORT>-unqualified-390.png（辅证，DOM 为准）
...
- npm run build：exit 0 ✓（新 BUILD_ID <redacted-build-id>）
...
### 边界核验
- HEAD=<redacted-hash> 未变；staged=0；未 push/commit/deploy
- 白名单外 68 项既有 dirty：双向相等（missing=0 extra=0）未动
- 原始 prisma/dev.db 未写（复制库 dev-lfc.db 隔离）；<PORT> 未改（只读记录）；<PORT> 隔离实例运行中（验证后可停）
