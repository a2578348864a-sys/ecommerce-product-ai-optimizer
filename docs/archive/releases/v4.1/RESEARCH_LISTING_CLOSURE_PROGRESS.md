...
## 任务 0（完成）
- 分支 feature/v4.1-ui-productization；HEAD <redacted-hash>；工作区已有用户未提交修改（TaskRecordDetail/ListingFactSupplementPanel/phase2StudioNavigation.test 等）——白名单外禁止触碰。
- prisma/dev.db SHA-256：<redacted-hash>（已存 before 快照）。
