...
## 基线
HEAD <redacted-hash>；staged=0；dirty=65（含白名单用户修改+既有 txt/png）；<PORT> PID <redacted>（next start -p <PORT>）；dev.db SHA <redacted-hash>… mtime 07:46（锁定态）；6 文件基线待跑。
...
- ✅ 白名单外修改 0（tmp 清理确认）；未提交/未推送/未部署/未写数据库；HEAD <redacted-hash> 未变。
...
## 活体验证（真实 <PORT> + 隔离库，Provider 完全关闭：baseURL=127.0.0.1:<PORT> 本机死端口 + 全部 AI_* key 清空）
...
- 浏览器（真实 Chrome CDP，1440×900 与 390×844）：Listing Studio 页显示 Listing 草稿五点数、描述、关键词；无横向溢出（scrollWidth<=innerWidth 两尺寸均 False）；console 0 error/0 unhandledrejection；网络资源请求 16 条全部 127.0.0.1:<PORT>，外部请求 0。
...
## 断点安全
- 未提交/未推送/未部署；HEAD <redacted-hash> 未变；<PORT> PID 未变（未停）；原 dev.db 只读未写；隔离库仅经 <PORT> 真实 API 写。
