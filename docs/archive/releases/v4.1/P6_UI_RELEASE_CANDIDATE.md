...
> 分支：feature/v4.1-ui-productization；基线 v4.0.1=<redacted-hash>；本报告仅记录可验证事实；
...
- 公网（https://your-server-ip）正式部署复验：**未完成（证据不足）——G7 发布授权未到**；已用相同构建 + public_showcase 模式在本地 <PORT> 完成等量语义验证。

## SHA / 分支 / 发布状态

| 项 | 值 |
|---|---|
| baselineSha | <redacted-hash>（v4.0.1） |
| validatedCodeSha（本轮） | <redacted-hash> |
...
**本地 Local Live（<PORT>，flag ON，桌面 1440 / 768 / 移动 390）**
...
**Public 语义（<PORT> = public_showcase + flag OFF 等量实例；同构建）**
...
- 详情：业务信息卡（关键词/市场/风险）、研究链路 8 阶段、证据来源（可展开）、高级详情折叠（74 步原始时间线保留）、Gate 5 / Guard 11 / hash <redacted-hash>…/ 脱敏通过 ✓（20-13-29-467Z.png）
...
- 证据目录：docs/archive/evidence/releases/v4.1/evidence/（before-* 基线 6 张）+ .playwright-cli/（20-0x… 验收 PNG/YML 多张）
