...
> 依据：当前代码（HEAD <redacted-hash> + 未提交 Formal v2 工作树，BUILD_ID <redacted-build-id>）逐项核对；只读审计，未做任何公网/外部调用。分类：公网已有 / 仅本地 / 缺实现 / 需配置 / 需领导授权。每项附当前代码或契约路径；历史文档目标不作为实现依据。
...
| 19 | 备份恢复 | 需配置 | docs/deployment/production-runbook.md; scripts/ops/db/protect-sqlite-db.mjs | 备份脚本存在；服务器侧定时备份/回滚演练需配置 |
