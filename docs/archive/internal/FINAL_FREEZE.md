...
原始冻结记录区分业务功能基线与仓库发布基线。具体机器信息、运行进程、构建编号、私有分支状态和部署细节已从公开副本移除；可公开核对的版本信息以 Git tags、`CHANGELOG.md` 和仓库历史为准。

| 维度 | 公开说明 |
| :--- | :--- |
| Git 分支与提交 | 以仓库当前 `main` 和 Git tag 为准 |
| 业务功能状态 | 已完成当期冻结范围的功能验收 |
| Working Tree | 冻结时按发布流程核对 |
| 运行与部署 | 由私有环境配置，不写入公开文档 |
...
- **真实 E2E 已验证**：用户已于本地 <PORT> 亲自完成真实图片生成 —— 研究事实（Human Confirmed Facts）→ Prompt（仅当前权威事实进入事实层）→ 真实 Provider → 图片结果，端到端验证通过；
...
原始冻结记录曾包含本地数据库路径、文件大小、修改时间、SHA-256 指纹和运行记录统计。这些属于私有环境信息，已从公开副本移除。数据库文件与未跟踪运行资产均不属于公开仓库内容。

`archives/` 等本地运行目录只由部署或验收环境管理，不应提交到 Git。
