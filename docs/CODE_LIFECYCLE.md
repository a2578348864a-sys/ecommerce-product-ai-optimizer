...
> Source baseline Commit：`835085bb0a464be41af4e40cc5672fbd666bdb0d`（短哈希 `835085b`）
> Source baseline Tree：`d57110cc0612d1992679c79d07819fa44bdbe8dc`
> 审计日期：2026-07-23
> 事实来源：已 fetch 的 `origin/main`，以及基于该基线创建的 `codex/opportunities-form-phase1b-leaf` 候选分支；以 tracked 文件、静态 import 图、Route 生命周期和 package/test 配置为依据。
...
> 候选边界：`lib/opportunityCandidateActions.ts` 与 Phase 1A 的 `OpportunitiesLockedPreview.tsx` 已在 production main；本页统计另纳入 Phase 1B 候选新增的 `components/cross-border/OpportunitiesDecisionSummary.tsx`。该文件合入 main 前仍是 `IN-FLIGHT / NOT_PRODUCTION`，不改变生产部署状态。
...
`PRODUCTION` 是 43 个非测试文件中除下列非生产项外的 28 个文件。核心包括 `AgentRunClient`、`OpportunitiesForm`、`OpportunitiesLockedPreview`、`OpportunitiesDecisionSummary`、`FamilyTop5Review`、`HomeDashboardClient`、`TaskRecordsList`、`TaskRecordDetail`、导航、登录与决策/Listing/图片卡片。`components/cross-border/OpportunitiesDecisionSummary.tsx` 合入 main 后为 `PRODUCTION / ACTIVE`；它由 `OpportunitiesForm` 的生产 Candidate pool 直接引用。
...
`lib/upstream/fixtures/` 的 8 个 JSON/Sidecar 文件属于 `PRODUCTION / PROTECTED_ARTIFACT`，不计入当前 139 个运行时代码统计：
