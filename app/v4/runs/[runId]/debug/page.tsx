 ...
 * V4.1 — 开发调试详情（研究后台）。
 *
 * 只从详情页顶部小字链「调试详情」进入，不进入任何导航；链接可见性低。
 * 本页面向开发者：展示原始 Run 事件流（全部原始节点/枚举/revision/hash/lastError/
 * wait 原始码/budget 明细），不涉及 C 端文案。普通页面不得出现这些内部字段。
 */
...
/**
 * 纯展示视图（不依赖路由/请求，便于单元测试）。
 * 传入原始 run/events/加载/错误态，输出原始字段与全部事件。
 */
