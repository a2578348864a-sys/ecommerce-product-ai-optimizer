...
const DEFAULT_REVIEW_TEXT = "待人工复核";
...
function parseAnalysisRequest(raw: unknown): { value?: AnalysisRequest; error?: ApiError } {
  if (!isRecord(raw)) {
...
function asReviewText(value: unknown) {
  return asString(value) || DEFAULT_REVIEW_TEXT;
}

function normalizeAiAnalysisResult(raw: unknown): AiAnalysisResult {
...
export async function POST(request: NextRequest) {
  ...
  if (contentLength > REQUEST_BODY_LIMIT_BYTES) {
    return jsonResponse({
      ok: false,
      error: { code: "body_too_large", message: "请求体过大，请减少商品描述或图片路径等内容。" },
    }, 400);
  }
