...
import { ResearchRunStore, type ResearchRunDb, parseState } from "@/lib/v4/runStore";
...
export async function GET(request: NextRequest, context: { params: Promise<{ runId: string }> }) {
  ...
  if (!state) return jsonError("run_not_found", "运行不存在。", 404);
  let events: unknown[] = [];
  try { events = JSON.parse(run.eventsJson); } catch { events = []; }
  return jsonOk({ run: state, events });
}
