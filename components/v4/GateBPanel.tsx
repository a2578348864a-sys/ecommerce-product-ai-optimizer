 ...
 * 四选项 content_ready / needs_information / revise_product / stop；stop 必填原因；
...
export type GateBChoice = "content_ready" | "revise_product" | "needs_information" | "abandon";

export const GATE_B_OPTIONS: GateBChoice[] = ["content_ready", "needs_information", "revise_product", "abandon"];

export const GATE_B_OPTION_LABELS: Record<GateBChoice, string> = {
  content_ready: "继续（进入内容制作）",
  needs_information: "返回补充信息",
  revise_product: "修改产品后重算",
  abandon: "放弃（不进入内容制作）",
};

export const GATE_B_OPTION_DESCRIPTIONS: Record<GateBChoice, string> = {
  content_ready: "认为已具备进入内容制作的条件，接受剩余未知与风险承担。",
  needs_information: "关键缺口仍需补充来源或确认，暂不进入内容制作。",
  revise_product: "当前产品商业可行性不足，调整方案后重算。",
  abandon: "放弃此产品，不再投入内容制作。",
};

export const GATE_B_OPTION_ICONS: Record<GateBChoice, typeof CheckCircle2> = {
  content_ready: CheckCircle2,
  needs_information: RefreshCw,
  revise_product: Pencil,
  abandon: LogOut,
};

/** 是否被 stale 规则阻断 content_ready（需要人工确认后才能继续）。 */
export function isProceedStaleBlocked(rulesStale: boolean, staleConfirmed: boolean): boolean {
  return rulesStale && !staleConfirmed;
}

/** 提交门禁：未选、stop 缺 reason、stale 未确认时的 content_ready 都不可提交。 */
export function canSubmitGateB(input: {
  ...
  if (option === "abandon" && !reason.trim()) return false;
  if (rulesStale && option === "content_ready" && !staleConfirmed) return false;
  return true;
}
...
export type GateBPanelProps = {
  ...
  actor: string;
  /** 规则 stale（reviewedAt 超过 90 天）→ 禁用 content_ready，需确认。 */
  rulesStale?: boolean;
  /** 人工确认“规则已过时仍继续”的回调。 */
  onConfirmStale?: () => void;
  /** 提交回调（只由人触发）。 */
  onSubmit: (payload: { option: GateBChoice; reason?: string; revision: number; actor: string }) => void;
  /** 整面板禁用（如提交中）。 */
  disabled?: boolean;
};
...
export function GateBPanel({
  ...
  const [option, setOption] = useState<GateBChoice | "">("");
    ...
    <section data-testid="gate-b-panel" className="surface-card rounded-2xl p-5">
      ...
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {GATE_B_OPTIONS.map((opt) => {
          ...
          const blocked = opt === "content_ready" && staleBlocked;
      ...
      {option && (
        <label className="mt-4 block">
          ...
          <textarea
            ...
            disabled={disabled}
            rows={3}
            placeholder={option === "abandon" ? "请填写停止原因（必填）" : "记录本次决策理由（可选）"}
          ...
          />
          {option === "abandon" && reason.trim().length === 0 && (
