...
export function SourcingEvidencePanel({
  ...
  const confirmedOffers =
    evidence?.candidates.filter((candidate) =>
      evidence.humanConfirmed.some((entry: HumanConfirmedEntry) => entry.offerId === candidate.offerId),
    ) ?? [];
  const hasConfirmedEvidence = confirmedOffers.length > 0;

  const [toolsOpen, setToolsOpen] = useState(!hasConfirmedEvidence);
  const userToggledToolsRef = useRef(false);

  useEffect(() => {
    if (!userToggledToolsRef.current) {
      setToolsOpen(!hasConfirmedEvidence);
    }
  }, [hasConfirmedEvidence]);

  useEffect(() => {
    if (status === "searching" || status === "saving" || Boolean(preview)) {
      setToolsOpen(true);
    }
  }, [status, preview]);
    ...
    <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4" data-testid="sourcing-evidence-panel">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasConfirmedEvidence ? (
            <span className="rounded-full border border-teal-200 bg-teal-50 px-2.5 py-0.5 text-xs font-semibold text-teal-700">
              已确认 {confirmedOffers.length} 条线索
            </span>
          ) : (
      ...
      {!accessReady ? (
        ...
        <>
          ...
          {/* ── 第一层：已取得的供应链证据（优先展示，证据主体） ── */}
          <div className="mt-3" data-testid="sourcing-evidence-primary-layer">
            {hasConfirmedEvidence ? (
              <div className="rounded-xl border border-slate-200 bg-white p-3" data-testid="sourcing-evidence-list">
                <p className="text-sm font-bold text-slate-700">已确认的供应线索（{confirmedOffers.length}）</p>
                <div className="mt-2 grid gap-2">
                  {confirmedOffers.map((candidate) => {
              ...
              <div
                className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-4 text-center"
                data-testid="sourcing-evidence-empty"
              >
                <p className="text-xs font-medium text-slate-500">暂无已确认的 1688 供应线索</p>
                <p className="mt-1 text-[11px] text-slate-400">可通过下方采集工具（关键词、图搜、链接）寻找货源并人工确认加入。</p>
              </div>
            )}
          </div>

          {/* ── 第二层：寻找与补充线索的 1688 采集工具（次级操作，折叠/独立分区） ── */}
          <details
            className="group mt-4 rounded-xl border border-slate-200 bg-slate-50/50 p-3.5 transition-colors open:bg-white"
            open={toolsOpen}
            onToggle={(e) => {
              userToggledToolsRef.current = true;
              setToolsOpen((e.currentTarget as HTMLDetailsElement)?.open ?? true);
            }}
            data-testid="sourcing-acquisition-tools"
          >
            <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-semibold text-slate-700 select-none [&::-webkit-details-marker]:hidden">
              <div className="flex items-center gap-2">
                <span className="inline-flex h-5 items-center justify-center rounded bg-slate-200 px-1.5 text-[10px] font-bold text-slate-700">
                  采集工具
                </span>
                <span className="font-medium text-slate-800">寻找与补充 1688 供应线索</span>
                <span className="hidden text-[11px] font-normal text-slate-500 sm:inline">
                  （关键词找货 / 图片找货 / 已有 1688 链接读取）
                </span>
              </div>
              <div className="flex items-center gap-1.5 text-xs text-slate-400 group-hover:text-slate-600">
                <span>{toolsOpen ? "收起工具" : "展开工具"}</span>
                <svg
                        ...
                        <div key={candidate.offerId} className="rounded-xl border border-slate-200 bg-white p-3" data-testid="sourcing-preview-card">
                          ...
                          {showDetail ? (
                            <div className="mt-2 rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs text-slate-600">
                              ...
                              <p className="mt-0.5">
                                卖家自报：{detail?.sellerClaims.slice(0, 3).map((claim) => `${claim.name}:${claim.value}`).join("；")}
                              </p>
                            </div>
                          ) : null}
                        </div>
