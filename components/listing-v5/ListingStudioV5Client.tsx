...
export function ListingStudioV5Client({ taskId }: { taskId: string }) {
  ...
  const run = async (action: "analyze_strategy" | "generate") => {
    ...
    try {
      const response = await fetch(`/api/tasks/${encodeURIComponent(taskId)}/listing-v5`, { method: "POST", headers: { "Content-Type": "application/json", ...buildAccessHeaders() }, body: JSON.stringify({ action }) });
      const json = await response.json().catch(() => null);
