...
const PREVIEW = {
  schema: "browser-evidence-collect.v1", version: 1,
  navigation: { finalUrl: "https://www.amazon.com/dp/B08NCVT244", navTarget: "task_asin", asin: "B08NCVT244", domainAllowed: true, navigationElapsedMs: 100 },
  calibration: { attempted: false },
  extraction: { pageStatus: "ok", entityBound: true, "capturedAt": "2026-08-14T02:00:00.000Z",
    fields: { asin: { value: "B08NCVT244", status: "correct", reason: null }, title: { value: "T", status: "correct", reason: null }, price: { value: 7.98, status: "correct", reason: null }, bsr: { value: null, status: "unknown", reason: null }, rating: { value: 4.4, status: "correct", reason: null }, reviewCount: { value: 8300, status: "correct", reason: null } } },
};
...
function byText(text: string): FakeElement | null {
  ...
  return walk(container as FakeNode);
}

let changedCount = 0;
const handleChanged = () => { changedCount += 1; };

async function mountSection(storageVersionValue: { resultJsonHash: string; updatedAt: string } | null) {
  changedCount = 0;
  await act(async () => {
    root = createRoot(container as unknown as Element);
    root.render(createElement(BrowserEvidenceSection, {
      taskId: "task-x", taskAsin: "B08NCVT244", evidence: null, storageVersion: storageVersionValue, capability: { state: "available" }, onChanged: handleChanged,
    } as never));
  });
  await flush();
...
async function rerenderVersion(storageVersionValue: { resultJsonHash: string; updatedAt: string }) {
  await act(async () => {
    root!.render(createElement(BrowserEvidenceSection, {
      taskId: "task-x", taskAsin: "B08NCVT244", evidence: null, storageVersion: storageVersionValue, capability: { state: "available" }, onChanged: handleChanged,
    } as never));
...
function bodyAction(init?: { body?: string }): string {
  if (!init || !init.body) return "";
  try {
    return String((JSON.parse(init.body) as Record<string, unknown>).action ?? "");
  } catch {
    return "";
  }
}

function versionEquals(actual: unknown, expected: unknown): boolean {
  return JSON.stringify(actual) === JSON.stringify(expected);
}

describe("BrowserEvidenceSection 保存冲突自动恢复（轮 12，真实组件挂载）", () => {
  it("首次保存 409 → 保留预览 + 刷新版本后安全重试一次 → 成功；成功后刷新资料", async () => {
    ...
    (globalThis as Record<string, unknown>).fetch = vi.fn(async (url: string, init?: { body?: string }) => {
      ...
      if (action === "collect") return makeResponse(true, 200, { ok: true, data: { preview: PREVIEW, evidenceId: "ev1" } });
      if (action === "save" && String(url).includes("browser-evidence")) {
        if (versionEquals((parsed as Record<string, unknown>).expectedStorageVersion, V1)) {
          return makeResponse(false, 409, { ok: false, error: { code: "task_result_conflict", message: "content changed" } });
        }
        return makeResponse(true, 200, { ok: true, data: {} });
      }
      return makeResponse(false, 404, { ok: false });
    });
    const api = await mountSection(V1);
    await act(async () => { byText("采集页面证据")?.dispatchEvent(new FakeEvent("click", byText("采集页面证据") as unknown as FakeNode)); });
    await flush();
    expect(findByTestId("workbench-browser-evidence")).not.toBeNull();
    expect(byText("我确认这是目标商品，保存证据")).not.toBeNull();
    await act(async () => { byText("我确认这是目标商品，保存证据")?.dispatchEvent(new FakeEvent("click", byText("我确认这是目标商品，保存证据") as unknown as FakeNode)); });
    await flush();
    // 首冲突：预览保留、onChanged 刷新资料
    expect(byText("我确认这是目标商品，保存证据")).not.toBeNull();
    expect(api.count()).toBeGreaterThanOrEqual(1);
    // 版本刷新后安全重试一次（V2）→ 成功
    await rerenderVersion(V2);
    await flush();
    expect(byText("我确认这是目标商品，保存证据")).toBeNull();
    expect(api.count()).toBeGreaterThanOrEqual(2);
    const saves = calls.filter((c) => (c.body as Record<string, unknown>).action === "save");
    expect(saves).toHaveLength(2);
    expect((saves[1].body as Record<string, unknown>).expectedStorageVersion).toEqual(V2);
  });

  it("二次仍 409：保留预览并显示「资料刚刚更新，请再试一次」，不无限重试", async () => {
    (globalThis as Record<string, unknown>).fetch = vi.fn(async (_url: string, init?: { body?: string }) => {
      if (bodyAction(init) === "collect") return makeResponse(true, 200, { ok: true, data: { preview: PREVIEW, evidenceId: "ev1" } });
      return makeResponse(false, 409, { ok: false, error: { code: "task_result_conflict", message: "content changed" } });
    });
    const api = await mountSection(V1);
    await act(async () => { byText("采集页面证据")?.dispatchEvent(new FakeEvent("click", byText("采集页面证据") as unknown as FakeNode)); });
    ...
    await rerenderVersion(V2);
    await flush();
    // 二次冲突：预览保留 + 简洁提示 + 不再重试（save 请求总次数=2，无第三次）
    expect(byText("我确认这是目标商品，保存证据")).not.toBeNull();
    expect(byText("资料刚刚更新，请再试一次。")).not.toBeNull();
    const fetches = (globalThis as unknown as { fetch: ReturnType<typeof vi.fn> }).fetch;
    const saves = fetches.mock.calls.filter((c: unknown[]) => bodyAction(c[1] as { body?: string } | undefined) === "save");
