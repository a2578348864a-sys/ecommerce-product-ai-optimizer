...
describe("ListingPreparationSummary (ReferenceListingDraftPanel) Contract & Retirement Check", () => {
  ...
  it("面板结构转换为「创作资料交接」摘要，包含5维统计、三层详情折叠区与文案工作台正式入口", () => {
    // 结构 ID 与 TestId
    ...
    // 面板标题与待复核徽章
    expect(panelSource).toContain("创作资料交接");
    expect(panelSource).toContain("✓ 已具备创作条件");
    // 统计项
    ...
    expect(panelSource).toContain("进入文案工作台");
    // 折叠依据详情与清单
  ...
  it("TaskRecordDetail 必须挂载于下一步区域（aria-label=下一步：创作资料交接），且 EvidenceWorkbench 不再包含它", () => {
    ...
    expect(taskDetailSource).toContain('aria-label="下一步：创作资料交接"');
