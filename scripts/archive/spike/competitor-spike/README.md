...
> `docs/archive/releases/v3/changes/phase-2/competitor-evidence-contract.md`）。
...
```bash
# 1) 生成空模板（可手填）
node scripts/archive/spike/competitor-spike/extract-template.mjs --template \
...
# 3) 校验输出
node scripts/archive/spike/competitor-spike/extract-template.mjs --validate .tmp/competitor-spike-1.json
```
...
- ✅ 允许：新增/修改 `scripts/archive/spike/competitor-spike/**`；按模板写 `.tmp/` 结果。
