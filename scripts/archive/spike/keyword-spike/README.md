...
> `docs/archive/releases/v3/changes/phase-3-4/proposal.md`）。
...
```bash
# 1) 生成空模板
node scripts/archive/spike/keyword-spike/extract-template.mjs --template \
...
# 3) 校验输出
node scripts/archive/spike/keyword-spike/extract-template.mjs --validate .tmp/keyword-spike-1.json
```
...
- ✅ 允许：新增/修改 `scripts/archive/spike/keyword-spike/**`；按模板写 `.tmp/` 结果。
