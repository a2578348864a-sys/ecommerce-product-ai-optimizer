...
// Final Capability: 真实浏览器最终能力闭环（规格六节）
// Mock Listing + 真实 product_visual（images.edit 参考图生图；授权 1 次）
  ...
  try {
    ...
    Object.assign(sanitizedEnv, {
      ...
      ...(() => {
        ...
        const envFile = "/path/to/workspace";
        if (!fs2.existsSync(envFile)) return {};
        const content = fs2.readFileSync(envFile, "utf8");
        const out: Record<string, string> = {};
        for (const key of ["OPENAI_API_KEY", "OPENAI_IMAGE_BASE_URL", "OPENAI_IMAGE_MODEL", "OPENAI_IMAGE_RESULT_HOSTS", "OPENAI_IMAGE_TIMEOUT_MS"]) {
