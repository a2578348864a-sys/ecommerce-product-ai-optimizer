...
vi.mock("@/lib/server/accessContext", () => ({ requireAuthenticated: vi.fn() }));
...
describe("Visitor task list DTO security", () => {
  ...
  it("不同 Visitor/Sandbox 不得获得可关联的回退项目键（�?id 不同沙箱 �?不同 ppk�?, async () => {
  ...
  it("正式工作台数据域：scope=product-research 不放�?mock / �?workflow 沙箱任务", async () => {
    state.sandboxTasks = [
    ...
    // 沙箱投影必须查询真实最�?run（ownerScope + sandboxId = demoAccessId�?
  ...
  it("Sandbox：同一 candidateId 返回两条 Candidate（身份都合法）→ 任务不得合并，且不泄露绑�?, async () => {
    ...
    const validIdentity = (suffix: string) => {
      try {
        // 直接调用 builder 保持�?owner 测试一致（避免测试内重复实现）
  ...
  it("Visitor run 真实投影：running/waiting/failed_recoverable/failed_terminal/cancelled/completed 正确，其�?Visitor �?run 不可�?, async () => {
    const hash = "a".repeat(64);
    ...
    // Visitor A �?run + 一条其�?Visitor �?run（隔离）
    state.runRows = [
    ...
    // 隔离：查询只�?Visitor A �?ownerScope/sandboxId
    expect(mockPrismaV4Runs()).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({ ownerScope: "visitor-a", sandboxId: "visitor-a" }),
    }));
  });

  it("Visitor stale 最高优先级（即使最�?run �?completed）→ research_stale，且 result �?status", async () => {
    const staleResult = {
      productName: "过期商品",
