...
function sellerSpriteDirectCandidate() {
  ...
  return {
    ...
    riskLevel: "unknown",
    riskLabel: "待核�?,
    summaryLabel: "SellerSprite 市场研究候�?,
    status: "pending",
...
function productBatchCandidate(scope: "owner:v1" | "visitor:sandbox" = "owner:v1") {
  ...
  return {
    ...
    riskLevel: "unknown",
    riskLabel: "需人工核验",
    summaryLabel: "SellerSprite市场研究候�?,
    status: "worth_analyzing",
...
beforeEach(() => {
  ...
  vi.clearAllMocks();
  mocks.prismaCreate.mockResolvedValue({ id: "task-owner-001", title: "桌面手机支架 一键分�? });
  mocks.txTaskCreate.mockResolvedValue({ id: "task-owner-001", title: "桌面手机支架 一键分�? });
  ...
  mocks.createTrustedSandboxTask.mockReturnValue({ id: "sandbox_task_001", title: "桌面手机支架 一键分�? });
...
describe("save-task runProof trust boundary", () => {
  ...
  it("rebuilds Candidate source metadata from authoritative server data", async () => {
    const body = signedBody({ candidateId: "candidate-a" });
    if (!body.sourceMeta) throw new Error("test fixture missing sourceMeta");
    body.sourceMeta.opportunityTitle = "客户端伪造标�?;
    ...
    expect(savedResult.sourceMeta.opportunityTitle).not.toBe("客户端伪造标�?);
