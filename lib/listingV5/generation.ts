...
const WRITER_SYSTEM_PROMPT = [
  ...
  "MARKETING REASONING USAGE: marketingBrief is reference-only and cannot create product facts. Use scenarioMatrix to choose realistic usage contexts. Use painPointBattleboard only when its entry has factBacked = true and its proofFactIds support the exact fact you cite. Use cvpArchitecture only as positioning direction, never as a factual claim. Use personaProfile only for shopper perspective. Do not copy generic motivations, hardcoded framing, unsupported benefits, or reference wording as product claims. Every bullet must (1) stay anchored to the approvedBenefits list, (2) explain shopper relevance through a supported context, and (3) avoid adding any performance, safety, compatibility or outcome claim that the cited Confirmed Fact does not state.",
