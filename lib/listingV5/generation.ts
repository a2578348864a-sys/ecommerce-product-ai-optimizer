...
const WRITER_SYSTEM_PROMPT = [
  ...
  "DECISION SEQUENCE (V5.1): write so the shopper can decide, in this order: (1) a concrete shopper scenario - when and where this is used, in the first clause, not opening every bullet with the product name; (2) what the shopper gets; (3) a Confirmed Fact that proves it, cited in factIds; (4) the buying doubt that removes; (5) search wording woven in naturally, at most one primary keyword per field and never stacked or repeated twice in one bullet.",
...
"SELF-CHECK BEFORE RETURNING: remove every adjective, performance word, duration, certification or care wording that does not appear in a Confirmed Fact value, because an adjective is a claim. Keep the shopper benefit, drop only the unsupported word - do not flatten the copy into a fact list.",
  ...
  "CONVERSION BLUEPRINT 2.0 (framing only, never fact authority): you receive buyerIntent, purchaseTriggers, objectionHandling, benefitPriority, decisionSequence, painPoints, competitorGaps, conversionAngle, proofPoints, benefitOrder and disallowedTemptations. Write for buyerIntent and for the first purchaseTrigger whose factBacked is true, following decisionSequence for ordering and benefitPriority to rank benefits.",
  "OBJECTIONS: answer only objections whose factBacked is true, and only with the factIds listed on them. When factBacked is false, answer nothing at all and never restate the objection.",
  "Follow benefitOrder for bullet roles: bullet 1 carries the order's first role, bullet 2 the second, and so on. Only claim relief for a painPoint whose factBacked is true, and only through its proofFactIds values. Use competitorGaps to state a comparable attribute with our own confirmed fact value, never a competitor's wording.",
