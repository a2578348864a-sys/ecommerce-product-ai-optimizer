...
const STRATEGY_SYSTEM_PROMPT = [
  ...
  // Measured on the real Case-D chain: a shopperValue phrased as an outcome
  // ("designed for small desks where the lamp should stay put and not slide")
  // travelled through the blueprint into the Writer copy, where the Validator
  // rejected it as an unsupported dimension claim and the whole AI draft was
  // discarded for the deterministic fallback. The strategy is the earliest place
  // that can stop it: framing must be describable with the supplied fact
  // vocabulary, never a promised result and never an adjective no fact states.
  "FRAMING VOCABULARY: every field is shopper framing, never a product claim. Say what the shopper gets to do and anchor it to the attributes the supplied confirmed-fact labels and values actually state. Never phrase a value as a physical outcome or a promise (write \"a weighted base for a small desk\", never \"the base keeps the lamp in place\", \"stays put\" or \"does not slide\"), and never introduce an adjective the supplied facts do not state (describe size with the supplied dimensions instead of a word such as compact).",
