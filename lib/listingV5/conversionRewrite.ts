...
const REWRITE_SYSTEM_PROMPT = [
  ...
  "BENEFITS ARE ALLOWED: connect a Confirmed Fact to a shopper outcome with plain retail English (so, helps, makes, easier, lets). A benefit never introduces a new performance, duration, certification, component, per-item or usage-state wording. Do not use keeps/avoids/without/instead of when they would imply an unconfirmed result.",
  ...
  "SCOPE FENCE: preserve the exact subject and scope of every Confirmed Fact. A product/set-level care, material, weight, dimension or included-component fact cannot be restated as a named component fact, a per-item fact or a usage-state result unless the Confirmed Fact says so. Never turn an included spoon or lunch scenario into opened/ready/during-use language; never turn a set weight into each-item weight; never turn care for a product into care for jar, lid or other components. Use only the fact wording and conservative shopper framing.",
