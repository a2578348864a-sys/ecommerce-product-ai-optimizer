...
export function buildSummaryPrompt(input: SummaryPromptInput) {
  return [
    ...
    "",
    "verdict 选择规则（从紧，不要轻易给「新手可小单测试」）：",
    "",
    "【禁止给「新手可小单测试」的情况】以下任一条件成立，verdict 不得为「新手可小单测试」：",
    "- sourcing 中 complianceBarrier=medium 或 high",
    "- sourcing 中 logisticsDifficulty=medium 或 high",
    "- sourcing 中 afterSalesRisk=medium 或 high",
    "- sourcing 中 beginnerFit=medium 或 low",
    "- sourcing 中 suggestedEntryLevel=intermediate 或 experienced",
    "- sourcing 中 feasibility=low",
    "- risk 中 overallLevel=yellow 或 red",
    "- risk 中 blacklistMatches 不为空",
    "- 商品涉及：婴儿/儿童/母婴/磁铁/带电/锂电池/食品接触/大件/承重/家具/美妆/医疗",
    "",
    "【只有同时满足以下所有条件才能给「新手可小单测试」】",
    "- beginnerFit=high 且 complianceBarrier=low 且 logisticsDifficulty=low 且 afterSalesRisk=low 且 suggestedEntryLevel=beginner",
    "- risk overallLevel=green 且 blacklistMatches 为空",
    "- 商品不涉及婴儿/儿童/母婴/磁铁/带电/锂电池/食品接触/大件/承重/家具/美妆/医疗",
    "",
    "【各品类 verdict 倾向】",
    "- 婴儿/儿童/母婴入口类 → 默认至少「有经验再做」；若 sourcing 显示合规信息不完整，给「新手不建议做」；若 risk 有 red，给「暂不建议做」",
    "- 磁铁/小零件儿童玩具 → 默认「新手不建议做」或「暂不建议做」",
    "- 食品接触类（水杯/餐具/饭盒）→ 默认至少「可做但需控制成本」；若涉及婴儿/儿童则至少「有经验再做」",
    "- 带电/锂电池类 → 默认至少「可做但需控制成本」；若同时涉及儿童/户外等风险叠加，至少「有经验再做」",
    "- 大件/家具/承重类 → 默认至少「可做但需控制成本」；若物流售后成本显著偏高，给「有经验再做」",
    "- 普通低风险轻小件（宠物配件/简单收纳/手机支架等）→ 可以「新手可小单测试」",
    "",
    "【兜底规则 — 非常重要】",
    "- 只要你不是 100% 确定满足「新手可小单测试」所有条件，就降一档给「可做但需控制成本」。",
    "- 宁可偏保守，不要因为信息不全就乐观。",
    "",
    "confidence 规则：",
    "- 只能用 高 / 中 / 低 之一。",
...
export function buildSourcingPrompt(input: SourcingPromptInput) {
  return [
    ...
    "",
    "【品类强制规则 — 必须严格执行，不可因为货源多就放水】",
    "",
    "婴儿/儿童/母婴入口类（包含 baby/infant/toddler/kids/children/child/teether/teething/pacifier/feeding/chew toy/婴儿/儿童/母婴/咬咬胶/安抚/入口/儿童餐具/牙胶 等关键词）：",
    "- beginnerFit 必须为 low（无论如何不能给 high，最多 medium 且仅当用户已提供认证信息时）。",
    "- complianceBarrier 必须为 high。",
    "- suggestedEntryLevel 必须为 experienced。",
    "- feasibility 不得为 high，最多 medium。",
    "- summary/risks 必须覆盖 CPC/儿童产品证书、ASTM F963/CPSIA/EN71 或目标市场对应标准、年龄标识、材质检测报告、供应商合规文件。",
    "",
    "磁铁/小零件/儿童玩具类（包含 magnet/magnetic/toy/building blocks/儿童玩具/磁力/磁铁/小零件/积木/拼装 等关键词）：",
    "- complianceBarrier 必须为 high。",
    "- beginnerFit 必须为 low。",
    "- suggestedEntryLevel 必须为 experienced。",
    "- feasibility 不得为 high。",
    "- risks 必须提到吞咽风险、小零件脱落、磁铁安全标准、年龄分级、CPC/ASTM F963/EN71/16 CFR 等。",
    "",
    "食品接触类（包含 cup/bottle/tableware/food contact/silicone cup/kitchen container/水杯/餐具/食品接触/硅胶水杯/饭盒/保鲜盒/水壶 等关键词）：",
    "- complianceBarrier 不得为 low，至少 medium。若同时涉及婴儿/儿童，则为 high。",
    "- suggestedEntryLevel 至少 intermediate。若同时涉及婴儿/儿童，则为 experienced。",
    "- summary/risks 必须提到食品接触材料合规、耐热耐温、气味挥发、FDA/LFGB/国标 对应标准提醒（仅提醒确认，不声称已通过）。",
    "",
    "带电/电池/锂电池/电子类（包含 battery/lithium/rechargeable/LED/light/lamp/electronic/electric/wireless/charger/露营灯/锂电池/电池/充电/电子/带电/灯具 等关键词）：",
    "- complianceBarrier 不得为 low，至少 medium。",
    "- logisticsDifficulty 不得为 low，至少 medium（电池产品有运输限制）。",
    "- suggestedEntryLevel 至少 intermediate。",
    "- summary/risks 必须覆盖 UN38.3 或电池运输限制、目标市场电子电气认证要求、平台电池类目审核。",
    "",
    "大件/承重/家具/户外桌椅类（包含 table/chair/furniture/folding table/camping table/heavy/oversized/折叠桌/露营桌/桌椅/家具/大件/承重/托盘 等关键词）：",
    "- logisticsDifficulty 不得为 low，至少 medium。",
    "- afterSalesRisk 不得为 low，至少 medium。",
    "- summary/risks 必须提到运输体积/重量/破损风险、退换货成本、承重宣称证据、包装成本。",
    "",
    `目标平台：${input.targetPlatform || "未提供"}`,
    "",
    "必须只返回合法 JSON object，不要 Markdown，不要代码块，不要解释文字。",
    ...
    "  * 婴儿/儿童/母婴入口类 → 强制 experienced",
    "  * 磁铁/小零件儿童玩具 → 强制 experienced",
    "  * 食品接触类 → 至少 intermediate（涉及婴儿则为 experienced）",
    "  * 带电/锂电池类 → 至少 intermediate",
    "  * 大件/家具/承重类 → 至少 intermediate",
    "  * 以上规则必须严格执行，不可因为货源多就降低",
