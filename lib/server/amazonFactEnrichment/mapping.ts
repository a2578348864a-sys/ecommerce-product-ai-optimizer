import type { AmazonSellerContentBlockV1, AmazonFactSourceV1, AmazonFactCandidateV1, AmazonFactField } from "./contract";
...
function sourceType(section: AmazonSellerContentBlockV1["section"]): AmazonFactCandidateV1["sourceType"] { return section; }
export function mapSellerBlocksToCandidates(input:{taskId:string;asin:string;blocks:AmazonSellerContentBlockV1[];structured?:Record<string,string>}, now=new Date().toISOString()):AmazonFactCandidateV1[] {
 ...
 const add=(field:AmazonFactField,value:string,source:AmazonFactSourceV1,approx=false,negative=false)=>{ const clean=normalizeSellerText(value,300); if(!clean) return; const key=`${field}:${clean.toLowerCase()}`; const existing=byKey.get(key); if(existing){existing.sources.push(source);existing.evidenceTexts.push(source.text);return;} const c:AmazonFactCandidateV1={id:`amazon-enrichment:${field}:${rows.length}`,taskId:input.taskId,asin:input.asin,field,value:clean,status:approx||negative?"review":"direct",sourceType:sourceType(source.section),sources:[source],evidenceTexts:[source.text],approximate:approx,negative,conflict:false,conflictReason:null,reviewRequired:approx||negative||field==="compatibility",createdAt:now}; byKey.set(key,c); rows.push(c); };
 ...
 const byField=new Map<AmazonFactField,AmazonFactCandidateV1[]>(); for(const c of rows){const arr=byField.get(c.field)||[];arr.push(c);byField.set(c.field,arr);} for(const arr of byField.values()) if(new Set(arr.map(c=>c.value.toLowerCase())).size>1){for(const c of arr){c.conflict=true;c.status="conflict";c.reviewRequired=true;c.conflictReason="multiple_values_same_field";}}
