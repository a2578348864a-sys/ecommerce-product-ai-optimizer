...
function buildTaskLinkedAiPrompt(input: {
  ...
  return [
    ...
    JSON.stringify({
      ...
      bulletPlans: (Array.isArray(input.plan.bulletPlans) ? input.plan.bulletPlans : []).map((b) => ({
        role: (b as { role?: string }).role ?? "core_outcome",
        shopperNeed: (b as { shopperNeed?: string }).shopperNeed ?? "",
        shopperAngle: b.shopperAngle,
        featureFactIds: b.featureFactIds,
        keywordIds: b.keywordIds ?? [],
        claimMode: (b as { claimMode?: string }).claimMode ?? "verified",
        cannotSay: (b as { cannotSay?: string[] }).cannotSay ?? [],
      })),
