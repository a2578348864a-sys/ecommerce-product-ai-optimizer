...
type TestContext = {
  source: ReturnType<typeof batch>;
  materials: ReturnType<typeof buildStage15SourceNativeEvaluationMaterials>;
  operator: ReturnType<typeof operatorResult>;
  screening: ReturnType<typeof buildStage15SourceNativeScreening>;
...
function createTestContext(): TestContext {
  ...
  const advanceProducts = new Set(screening.screening.items.filter((item) => item.status === "advance").map((item) => item.productKey));
  ...
  return { source, materials, operator, screening, advanceIds };
}
...
function outcome(context: TestContext, role: "a" | "b", advanceYes: number, nonAdvanceYes: number, insufficient = 0, evidenceYes = 20) {
  const template = role === "a" ? context.materials.outcome.assessorA.template : context.materials.outcome.assessorB.template;
  ...
  // Outcome IDs are deliberately shuffled, so use the already verified context bindings to target the five actual advances.
  const advanceAnswers = answers.filter((answer) => context.advanceIds.has(answer.evaluationItemId));
  const nonAdvanceAnswers = answers.filter((answer) => !context.advanceIds.has(answer.evaluationItemId));
...
function changeAgreementWithoutChangingThresholdMetrics(result: ReturnType<typeof outcome>, context: TestContext, nonAdvanceChanges: number) {
  const advance = result.answers.filter((answer) => context.advanceIds.has(answer.evaluationItemId));
  const nonAdvance = result.answers.filter((answer) => !context.advanceIds.has(answer.evaluationItemId));
...
function input(context: TestContext, results: ReturnType<typeof outcome>[], roleAttestations = roles()) {
  return {
    batch: context.source,
    materials: context.materials,
    screening: {
      kind: "task7_artifact" as const,
      artifact: context.screening,
      trustedInput: { batch: context.source, materials: context.materials, operatorResult: context.operator },
    },
    outcomeAssessorResults: results,
    roleAttestations,
  };
}
...
describe("Stage 1.5 source-native effectiveness", () => {
  it("preserves exact boundary numerators and supports only the complete two-assessor threshold", () => {
    ...
    const result = analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 3, 0, 1), outcome(context, "b", 3, 0, 1)]));
    ...
    expect(() => assertStage15SourceNativeEffectivenessIntegrity(result, input(context, [outcome(context, "a", 3, 0, 1), outcome(context, "b", 3, 0, 1)]))).not.toThrow();
  });
  ...
  it.each([
    ...
    const context = createTestContext();
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", advanceYes, nonAdvanceYes, insufficient), outcome(context, "b", shared ? advanceYes : 3, shared ? nonAdvanceYes : 0, shared ? insufficient : 1)])).conclusion).toBe(expected);
  });

  it("uses only directional for a complete single assessor or overlapping roles", () => {
    const context = createTestContext();
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0)])).conclusion).toBe("directional_workflow_signal_observed");
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0), outcome(context, "b", 5, 0)], roles({ outcomeAssessorsDistinct: false }))).conclusion).toBe("directional_workflow_signal_observed");
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [])).conclusion).toBe("evaluation_inconclusive");
  });

  it("keeps the exact agreement threshold at 15/20 and maps 14/20 to inconclusive", () => {
    const context = createTestContext();
    const assessorA = outcome(context, "a", 3, 0, 1);
    const exact15 = changeAgreementWithoutChangingThresholdMetrics(outcome(context, "b", 3, 0, 1), context, 2);
    const atThreshold = analyzeStage15SourceNativeEffectiveness(input(context, [assessorA, exact15]));
    ...
    const belowThreshold = analyzeStage15SourceNativeEffectiveness(input(context, [assessorA, changeAgreementWithoutChangingThresholdMetrics(outcome(context, "b", 3, 0, 1), context, 3)]));
  ...
  it("maps no positive direction, low agreement, and substantial evidence gaps deterministically", () => {
    const context = createTestContext();
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 0, 0), outcome(context, "b", 0, 0)])).conclusion).toBe("screening_workflow_signal_not_observed");
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 2, 7), outcome(context, "b", 2, 7)])).conclusion).toBe("screening_workflow_signal_not_observed");
    const lowAgreement = analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0), outcome(context, "b", 0, 5)]));
    ...
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0, 0, 9), outcome(context, "b", 5, 0, 0, 20)])).conclusion).toBe("evaluation_inconclusive");
  });

  it("blocks identity hard gates and fails closed for self-consistent packet, hash, or id forgeries", () => {
    const context = createTestContext();
    expect(analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0), outcome(context, "b", 5, 0)], roles({ identityHardGateReasons: ["blindness_breach"] }))).conclusion).toBe("blocked");
    const data = input(context, [outcome(context, "a", 5, 0), outcome(context, "b", 5, 0)]);
    ...
    const badId = outcome(context, "a", 5, 0);
  ...
  it("reports descriptive kappa for a known three-category fixture", () => {
    const context = createTestContext();
    const analysis = analyzeStage15SourceNativeEffectiveness(input(context, [outcome(context, "a", 5, 0), outcome(context, "b", 5, 0)]));
